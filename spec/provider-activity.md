...
- `access` (optional): classifies a RESOURCE frame, `"mcp"` when the node is an
  ...
  opencode `write` / `edit`, Codex `apply_patch`, ...), the resolver
...
5. `shell` (+ levels below): paths parsed HEURISTICALLY out of shell
   commands (2026-08-17: a `PreToolUse` hook on `Bash`, declared by the
   providers whose runtime reports that claude-convention shape, claude
   from day one and codex since 2026-08-18 (0.147 reports its shell
   tool as `Bash`, live-verified); rendered ONLY when the operator
   opted in at install time,
