...
| `GET /api/activity/sessions` | implemented | Session-journal read-back for client hydration (contract: [`provider-activity.md` §Session journal](./provider-activity.md#session-journal)). `{ schemaVersion: '1', kind: 'activity-sessions', sessions: SessionRecording[], skipped: string[], recording: boolean }`: files read fresh per request in name order, AJV-validated against `session-recording.schema.json`, off-shape basenames listed in `skipped` (map-views dialect); pending journal buffers flush first so a just-recorded session is visible; `recording` is the live capture state so a reloaded page restores its Record/Stop control. No token (operator UI surface). |
