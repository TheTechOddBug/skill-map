...
| `POST /api/nodes/:pathB64/findings/:id/dismiss` | implemented | HTTP face of `sm findings dismiss` for the inspector's per-finding X (a DIRECT action, no note; the CLI's `--note` stays terminal-only). Body `{ confirm?, always? }`: writes the `annotations.suppressions` entry to the node's `.sm` through the SAME gated channel as the action dispatch routes (no standing consent → `412` `confirm-required` with `details.key = 'allowEditSmFiles'`, the UI consent dialog retries with `confirm` / `always`; `always` persists the grant and reloads the server config cache) and refreshes the write-through `scan_nodes.annotations_json` mirror, read-time lens semantics (rows kept, hidden). Kernel safety-lane findings are not dismissible → `409` `finding-not-dismissible`. Unknown id, id on another node, or missing DB → `404`. Success `204 No Content`; the client re-fetches the tray. |
...
| `GET /api/nodes/:pathB64/prob-extensions` | implemented (Step 16) | The node's probabilistic launcher list (`kind: "node.prob-extensions"`) for the inspector's finder buttons, per [`ROADMAP.md` §Step 16](../ROADMAP.md). Two buckets: `finders` (probabilistic Analyzers whose precondition matches the node AND that have ≥ 1 matching fixer, i.e. non-empty `fixerIds`) and `standalone` (probabilistic Analyzers matching the node with NO fixer, PLUS probabilistic Actions without `analyzerIds`). The former separate `fixers` bucket is RETIRED: a fixer never gets its own launcher, it surfaces through its finder's `fixerIds` and the tray's per-finding fix button submits it. Each entry: `id`, `description`, live `state` (`idle` / `queued` / `running`, from `state_jobs` over the button's actions `{id} ∪ fixerIds` so a queued/running FIXER lights the finder button, running wins over queued), `jobId` (that active job id, `null` when idle, the stop handle), `lastJudged: { at, model } | null`, `fixerIds` (qualified ids of the Actions whose `analyzerIds` name this finder, via the shared inverse-Modelo-B resolver `matchesQualifiedExtensionFilter`; empty on standalone entries), and `hasOpenFindings` (true when the node carries an UNRESOLVED non-stale finding of this finder's extension id; it DISABLES the finder button, re-running a finder with open findings is pointless, the fix / resolve / dismiss actions live on each finding row and the button re-enables once none is open). The boot-cached plugin runtime is reused (audit M3, no per-request disk walk), but the ENABLED set is re-read per request via a fresh resolver over the live layered config (`core/runtime/fresh-resolver.ts`), so a plugin toggled mid-session, e.g. a new project enabling the experimental AI actions, appears WITHOUT restarting `sm serve` (built-ins are always in memory; only drop-ins that booted `startsAsDisabled` still need a restart). 404 rules as above. |
