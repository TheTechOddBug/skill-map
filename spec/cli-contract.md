...
The consequence for scoping: **there is deliberately no single-node scan**, and `sm enrich <node.path>` is NOT one. Graph-wide Analyzers (broken references, orphans, duplicate names) read the whole corpus, so a node scanned in isolation would produce wrong issues for that node AND leave every other node's issues stale. Positional arguments to `sm scan` are ROOTS (directories); passing a file is a usage error naming the two verbs that do narrow. The incremental path is `sm scan --changed`, which still walks and still re-runs every Analyzer over the merged graph, but re-extracts only the nodes whose content moved.
