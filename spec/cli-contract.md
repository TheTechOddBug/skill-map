...
| `GET /api/map-views` | implemented | Named map views of the project ([`map-views.md`](./map-views.md)), read fresh from `<cwd>/.skill-map/views/*.json` on every request (no cache, no watcher). Shape: `{ schemaVersion: '1', kind: 'map-views', views: [{ slug, view: MapView }], skipped: string[] }`, entries sorted by slug, each `view` validating against [`schemas/map-view.schema.json`](./schemas/map-view.schema.json). A file that fails JSON parse or schema validation is SKIPPED, its basename listed in `skipped`, and logged (warn); a hand-edited broken view must never take the whole list down. Absent `views/` directory → empty list. |
