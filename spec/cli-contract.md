...
| `POST /api/agent/doorbell` | implemented | Wake-endpoint registration for the agent doorbell ([`job-lifecycle.md` §Agent doorbell](./job-lifecycle.md)). Body `{ url }`, the runtime's local HTTP API base (OpenCode's generated activity plugin registers its own `serverUrl` at plugin load). Same session-token header + loopback source gate as `POST /api/activity`; a non-loopback `url` is refused (422 `bad-query`). In-memory and boot-scoped (dies with the server, last write wins); the registration also refreshes via the optional `agentEndpoint` field on the activity ingest body, so a restarted server relearns it from ordinary traffic. Responses: `202` `{ ok: true }`, `403 token-mismatch`, `400 bad-query`. |
