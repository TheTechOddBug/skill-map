...
| Verb | Behavior |
|---|---|
| `sm agent install [--for <provider>]` | Materialise the canonical `sm-process-jobs` skill folder into the lens's `scaffold.skillDir` (`<skillDir>/sm-process-jobs/`; same destination selection as `sm tutorial`, `--for` overrides the active lens). The folder is a set of files (`SKILL.md` entry plus the per-mode `mcp.md` / `cli.md` resources, see below), treated atomically: fresh install (the entry file was absent) writes the whole set; when any file differs OR is missing against the canonical set (an older CLI that shipped only `SKILL.md`, a manual edit) the set is rewritten and reported as an update; every file identical is a no-op reported as already up to date. The outcome wording mirrors the UI affordance (Install / Update / Up to date), both driven by the same set comparison the `status` verb's `stale` field uses. A Provider without `scaffold.skillDir` is refused with exit 2 and a directed advisory. |
...
| `GET /api/agent/presence` | implemented | `{ schemaVersion: '1', kind: 'agent-presence', attending: boolean, lastClaimAt: number \| null }`. Whether a processing agent has been OBSERVED attending this project's queue since the server started. `attending` is STICKY: it flips true on the first `job.claimed` the server sees, OR on the first MCP `claim_job` ATTEMPT (job or no job: an agent parked on `claim_job { wait }` against an empty queue claims nothing for hours, yet it is attending by definition, it is literally asking for work), and never flips back on SILENCE, because a parked agent claims only when work arrives, so silence proves nothing and a TTL would manufacture false negatives. The one thing that flips it back is EXPLICIT negative evidence: a `core/ai-ping-action` job observed submitted and then cancelled while still unclaimed (the manual Check probe's timeout path: the operator asked "anyone there?" and nobody answered). Evidence ordering is exact (monotonic, not timestamps); any later claim or claim attempt flips it true again. A cancel of a CLAIMED ping, or of any ordinary job, is queue housekeeping and never counts. `lastClaimAt` stays claim-only; an attempt never forges a timestamp. `lastClaimAt` is the epoch-ms of the most recent claim (display only). Both paths count: an MCP `claim_job` broadcasts in-process, and a CLI `sm jobs claim` pushes `job.claimed` to `POST /api/job-events`, so a CLI-parked agent (invisible to any MCP-session count) is observed exactly like an MCP one. To learn it without waiting for organic traffic, a server MAY submit ONE liveness ping at startup and cancel it when it goes unclaimed; the ping is only traffic to provoke an observation, the answer still comes through the same claim path. Ephemeral: the state dies with the process, like the live-activity stats. |
| `GET /api/mcp/status` | implemented | `{ schemaVersion: '1', kind: 'mcp-status', enabled: boolean, connected: boolean, clients: number, url: string }`. Live MCP-connection probe behind the Quick Start "MCP installed on your agent" check. `enabled` is the resolved `mcp.server.enabled` (same value as `/api/health`'s `mcp`); `connected` is `true` when at least one client holds a live stateful `/mcp` session (`McpSessionManager.sessionCount > 0`), `clients` is that count. SCOPE-AGNOSTIC and reads no `$HOME`: it sees the live session no matter which Claude Code scope (local / project / user) registered the server, complementing the passive `mcp://skill-map` graph scan (which only sees a project `.mcp.json`). `url` is the endpoint a client registers, built by the server from its OWN bind (`http://<host>:<port>/mcp`, a wildcard bind reported as loopback since that is what a local agent can dial). It is authoritative and the page origin is NOT a substitute: under a split dev setup the SPA is served by a proxy whose port is not the one `/mcp` listens on, so a UI that composed the URL from its own origin would hand the operator a dead address. MCP off → `enabled: false`, `connected: false`, `clients: 0` (`url` still reports where `/mcp` WOULD answer once enabled). |
...
| `GET /api/branch?path=<p>&exclude=<p>&excludeRoot=<1\|0>&limit=<n>` | implemented | Branch projection for the map, scoped by **map scope overrides** (see §Map scope overrides for the evaluation rule). `path` (repeatable) carries the INCLUDE overrides, `exclude` (repeatable) the EXCLUDE overrides, `excludeRoot` the root override (`1` excluded, `0` included; any other value 400 `bad-query`); empty values are filtered, duplicates de-duped. When `excludeRoot` is ABSENT, the root is inferred excluded iff at least one `path` value has no strict ancestor among the `exclude` values, so the historical forms keep their meaning byte-for-byte: no params = whole corpus, bare `?path=app` = only the `app/` subtree. A path present in BOTH `path` and `exclude` → 400 `bad-query`. The scoped set is capped at `limit` nodes (default and effective max = the scan's `maxRenderNodes`); `total` / `truncated` are computed AFTER override evaluation, before the cap. Direct shape (no envelope wrap, like `/api/scan`): `{ schemaVersion, kind: 'branch', branch: { paths, excluded, rootExcluded, total, rendered, truncated, cap }, nodes: Node[], links: Link[], issues: Issue[] }`, where `paths` echoes the de-duped includes and `excluded` / `rootExcluded` echo the RESOLVED scope (post-inference). `nodes` is the first `rendered` nodes of the scoped set in stable path order; `links` carries only edges whose source AND **resolved target** are in `nodes` (the resolved target is `resolvedTarget`, the node a trigger-style `invokes` / `mentions` link points to, falling back to the raw `target` for path-style links; a genuinely-broken link whose target resolves to no node is excluded); `issues` carries those touching `nodes`. `truncated` is `total > cap`. Lets the SPA render the rail's checkbox state without hydrating the full `ScanResult`. DB absent → empty branch (zero nodes) echoing the resolved scope. Validation: `limit` integer ≥ 1 else 400 `bad-query`. |
