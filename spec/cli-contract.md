...
Actions are not invoked via `sm actions`. Probabilistic Actions queue via `sm job submit` (below); deterministic enrichment Actions (`io: ['network']` + an `enrichments/` report schema) execute via `sm refresh`; the remaining deterministic Actions dispatch in-process (`POST /api/actions/:id`, `sm bump`, ...).
