...
| `sm findings [-n <node.path>] [--extension <ids>] [--type <slug>] [--severity <s>] [--since <iso>] [--threshold <0..1>] [--stale]` | Read `state_findings`: the judgments recorded by probabilistic finder Analyzers plus the kernel-derived safety rows (`injection-detected` / `content-suspicious` / `content-malformed`). Filters: `-n` by node path; `--extension` comma-separated qualified or short extension ids (same matching as `sm check --analyzers`); `--type` by finding slug; `--severity` minimum severity; `--since` ISO date on `generated_at`; `--threshold` minimum confidence. Stale rows (body hash drifted since generation) are EXCLUDED by default; `--stale` includes them, marked. Exit 0 regardless of content: findings are probabilistic, advisory by construction, and never drive exit codes (the deterministic sibling with exit-code semantics is `sm check`). `--json` emits `{ ok, kind: 'findings', findings[], total }`, each entry mirroring the `state_findings` row (camelCase, including `model` when the recording agent declared one) plus a derived `stale` boolean. Human mode renders the model name alongside the confidence when present. See [`db-schema.md` §state_findings](./db-schema.md). |
