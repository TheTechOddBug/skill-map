...
| `sm show <node.path>` | Node detail: weight (tokens triple-split), frontmatter, links in/out, issues, findings, and any stored per-node summary. A stored summary (written by `sm record` for a summarizer Action, one whose report schema extends `summaries/<kind>.schema.json`, see [`job-lifecycle.md` §Record](./job-lifecycle.md#record-callback)) renders in a `Summary` section as the summarizer action id + the report headline, marked `(stale)` when the node's body changed since the summary was generated (`body_hash_at_generation` differs from the current `scan_nodes.body_hash`). `--json` emits a detail object with the raw link rows plus a `summaries` array, each entry carrying a `stale` boolean. Pretty output groups identical-shape links (same endpoint, kind, normalized trigger) onto one line and lists the union of extractor ids in a `sources:` field; the section header reports both the raw row count and the unique-after-grouping count, e.g. `Links out (12, 9 unique)`. Storage keeps one row per extractor (`scan_links` unchanged); grouping is purely read-time presentation. |
