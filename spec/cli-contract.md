...
| `sm plugins enable <id>... \| --all [--local]` | Operational toggle ON. Persists the per-extension `enabled` in the config layers (`plugins.<id>.extensions.<ext>.enabled`), defaulting to the shared `settings.json`; `--local` writes `settings.local.json` instead. Does NOT grant import trust (use `sm plugins trust`). Accepts one or more ids; batches are all-or-nothing (any unknown / mismatched id aborts before any write) and repeated ids are deduped. `--all` applies to every discovered plugin. **Pair toggle**: enabling a fixer Action also enables the analyzer(s) in its `precondition.analyzerIds`, and enabling an analyzer also enables the fixers referencing it (direct edges only; companions are reported as informational `pair toggle:` lines, never re-prompt, follow the same `--local` target, and locked companions are skipped silently). See [`plugin-author-guide.md` §Paired extensions](./plugin-author-guide.md#paired-extensions-pair-toggle). `--json` emits `{ ok, kind: 'plugins-toggle', action, ids, elapsedMs }`, where `action` is `enable` / `disable` and `ids` lists every qualified extension id actually flipped, pair-toggle companions included (the macro-confirm and companion lines stay on stderr). |
