...
| `sm findings [-n <node.path>] [--extension <ids>] [--type <slug>] [--severity <s>] [--since <iso>] [--threshold <0..1>] [--stale] [--fixed] [--dismissed]` | Read `state_findings`: the judgments recorded by probabilistic finder Analyzers plus the kernel-derived safety rows (`injection-detected` / `content-suspicious` / `content-malformed`). Filters: `-n` by node path; `--extension` comma-separated qualified or short extension ids (same matching as `sm check --analyzers`); `--type` by finding slug; `--severity` minimum severity; `--since` ISO date on `generated_at`; `--threshold` minimum confidence. The default view shows what needs attention (open and `human-decision` rows), hiding three disjoint kinds of row: **dismissed** rows (their (extension, type) class matches an active sidecar suppression, the read-time lens, [`db-schema.md` §state_findings](./db-schema.md#state_findings); TOP precedence, a suppressed row counts as dismissed no matter its resolution or staleness), **`fixed`** rows (`resolution = 'fixed'`, already handled) and **stale** rows (body hash drifted since generation; a fixed-and-stale row counts as fixed, state precedence). `--dismissed` shows ONLY the suppressed bucket; `--fixed` shows ONLY the fixed bucket (marked with `resolution_actor` (`fixer` / `human`), the fixer id and its note); `--stale` shows ONLY the stale bucket (marked); together, only their union. The bucket flags are FILTERS, not additive reveals: with any present the needs-attention rows are omitted, and the excluded-count reporting below does not apply (it exists to keep the DEFAULT view honest; an explicit bucket filter is the operator's own narrowing, exactly like `--type`). A bucket-filtered view whose query matched rows but none land in the requested bucket renders a neutral no-match line, never the clean-verdict line; a query returning no rows at all keeps the clean verdict (still the only clean-verdict output). Open rows and **`human-decision`** rows always show (a `human-decision` finding is the fixer's proposal awaiting the author's choice, the highest-priority TODO; a suppressed one hides as dismissed). **Excluded rows MUST be reported, never silently swallowed**: an empty result reads `No fresh findings` plus the hidden breakdown and its remedy (`N dismissed`, `M fixed`, `K stale`), NEVER a bare `No findings` (which asserts a clean node while rows sit hidden); a populated listing footers the same breakdown; and any hidden non-suppressed row that is `human-decision` is named explicitly (a proposal staled by a sibling fix would otherwise vanish). `--json` carries `dismissedExcluded`, `fixedExcluded` and `staleExcluded` counts. A `fixed` row is a state, NOT a verdict: it records that a fix happened, never that the defect is verified gone, re-running the finder is how the operator confirms (clean deletes it, still-present reopens it). A node with no rows at all is the only clean-verdict output. Exit 0 regardless of content: findings are probabilistic, advisory by construction, and never drive exit codes (the deterministic sibling with exit-code semantics is `sm check`). `--json` emits `{ ok, kind: 'findings', findings[], total, dismissedExcluded, fixedExcluded, staleExcluded }`, each entry mirroring the `state_findings` row (camelCase, including `model` and the `resolution*` fields, `resolutionActor` included, when set) plus a derived `stale` boolean. Human mode prefixes each finding row with its numeric id (right-aligned per section, the handle for `sm findings resolve <id>`) and renders the model name alongside the confidence when present. See [`db-schema.md` §state_findings](./db-schema.md). |
...
| `sm findings dismiss <finding.id> [--note <text>]` | Silence a finding the operator has judged acceptable (a false positive, or an intentional pattern the finder keeps flagging). Durable, not a row state: it writes a standing `annotations.suppressions` entry to the node's `.sm` sidecar (through the gated sidecar write channel, same consent as `sm bump`) keyed by the finding's emitting extension and `type`, and refreshes the write-through `scan_nodes.annotations_json` mirror. The rows are NOT deleted: the suppression is a READ-TIME lens ([`db-schema.md` §state_findings](./db-schema.md#state_findings)), the judgment CLASS hides from the default view (`--dismissed` reveals it, reported as `N dismissed` in the excluded breakdown), finder re-runs keep judging and recording it (hidden), and `sm findings undismiss` restores visibility instantly (suppression grain is per (extension, type), findings have no stable cross-run identity). Kernel safety-lane findings (`injection-detected` / `content-suspicious` / `content-malformed`) are NOT dismissible (exit 2). Exit 5 if the id does not exist. `--json` emits the written suppression entry. Distinct from `resolve` (which marks a finding FIXED, a resolution) and from `prune` (which clears stale rows): dismiss says "this judgment does not apply here, stop showing it".
...
| `GET /api/nodes/:pathB64/findings` | implemented (Step 16 piece 1) | The node's `state_findings` rows as a list envelope (`kind: "findings"`), mirroring the `sm findings -n <path>` default view: FRESH open rows in `items` (severity, `type`, `message`, `detail`, `confidence`, `model`, `extensionId`/`Version`, `origin`, resolution fields, `generatedAt`, `id`), with the excluded-count honesty carried on `counts` (`dismissedExcluded`, `fixedExcluded`, `staleExcluded` beside `total` / `returned`) so the UI can render the same "N dismissed, M fixed, K stale hidden" line as the CLI. The dismissal lens reads the write-through `scan_nodes.annotations_json` mirror in the same DB open (zero file reads). Query flags `?dismissed=1` / `?fixed=1` / `?stale=1` are bucket FILTERS (only that bucket, mirroring the CLI flags). Malformed `pathB64` / unknown node / missing DB → 404 `not-found`. |
| `GET /api/nodes/:pathB64/prob-extensions` | implemented (Step 16) | The node's probabilistic launcher list (`kind: "node.prob-extensions"`) for the inspector's two-state finder buttons, per [`ROADMAP.md` §Step 16](../ROADMAP.md). Two buckets: `finders` (probabilistic Analyzers whose precondition matches the node AND that have ≥ 1 matching fixer, i.e. non-empty `fixerIds`) and `standalone` (probabilistic Analyzers matching the node with NO fixer, PLUS probabilistic Actions without `analyzerIds`). The former separate `fixers` bucket is RETIRED: a fixer is the second state of its finder's button, never its own launcher. Each entry: `id`, `description`, live `state` (`idle` / `queued` / `running`, from `state_jobs` over the button's actions `{id} ∪ fixerIds` so a queued/running FIXER lights the finder button, running wins over queued), `jobId` (that active job id, `null` when idle, the stop handle), `lastJudged: { at, model } | null`, `fixerIds` (qualified ids of the Actions whose `analyzerIds` name this finder, via the shared inverse-Modelo-B resolver `matchesQualifiedExtensionFilter`; empty on standalone entries), and `hasOpenFindings` (true when the node carries an UNRESOLVED non-stale finding of this finder's extension id, driving the Detect ⇄ Fix morph). The boot-cached plugin runtime is reused (audit M3, no per-request disk walk), but the ENABLED set is re-read per request via a fresh resolver over the live layered config (`core/runtime/fresh-resolver.ts`), so a plugin toggled mid-session, e.g. a new project enabling the experimental AI actions, appears WITHOUT restarting `sm serve` (built-ins are always in memory; only drop-ins that booted `startsAsDisabled` still need a restart). 404 rules as above. |
...
| Request body | `{ "extension": <string, required>, "autoFix"?: <boolean, default false> }` (JSON, AJV). `extension` is a qualified or bare probabilistic extension id (same matching as the CLI). `autoFix: true` on a finder submit freezes `state_jobs.auto_fix` so `sm record` chains the finder's fixers on completion (see [`job-lifecycle.md` §Auto-fix chain](./job-lifecycle.md)); the inspector's automatic-toggle finder button sends it. |
