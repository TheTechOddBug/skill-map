...
| `3` | Duplicate conflict | Job submission refused because an active duplicate exists (same `extension + version + node + contentHash`). Returned by `sm job submit`. |
...
- **Non-drift-owning WRITE verbs** (`sm job submit` / `cancel` / `fail` / `prune`, `sm record`, `sm findings prune`, any verb inserting or mutating rows) MUST refuse cleanly on either drift axis with the drift advisory (exit `2`), before loading the plugin runtime and before touching any table: writing into a drifted schema risks partial or corrupt rows, and secondary reads during boot (e.g. the plugin trust store) silently misbehave on a drifted DB. Rationale: observed live, `sm job submit` against a drifted DB reported `extension <id> not found` because the trust read degraded to untrusted, a misleading symptom three layers away from the cause. The advisory names the actual problem and its remedy.
...
| `sm findings [-n <node.path>] [--extension <ids>] [--type <slug>] [--severity <s>] [--since <iso>] [--threshold <0..1>] [--stale] [--fixed]` | Read `state_findings`: the judgments recorded by probabilistic finder Analyzers plus the kernel-derived safety rows (`injection-detected` / `content-suspicious` / `content-malformed`). Filters: `-n` by node path; `--extension` comma-separated qualified or short extension ids (same matching as `sm check --analyzers`); `--type` by finding slug; `--severity` minimum severity; `--since` ISO date on `generated_at`; `--threshold` minimum confidence. The default view shows what needs attention, hiding two disjoint kinds of row: **`fixed`** rows (`resolution = 'fixed'`, a fixer already handled them, `--fixed` includes them, marked with the fixer id and its note) and **stale** rows (body hash drifted since generation, `--stale` includes them, marked). Open rows and `declined` rows always show (a declined finding is the author's pending decision). **Excluded rows MUST be reported, never silently swallowed**: an empty result reads `No fresh findings` plus the hidden breakdown and its remedy (`N fixed`, `M stale`), NEVER a bare `No findings` (which asserts a clean node while rows sit hidden); a populated listing footers the same breakdown; and any hidden row that is `declined` is named explicitly (a declined finding staled by a sibling fix would otherwise vanish). `--json` carries `fixedExcluded` and `staleExcluded` counts. A `fixed` row is a state, NOT a verdict: it records that a fixer ran, never that the defect is verified gone, re-running the finder is how the operator confirms (clean deletes it, still-present reopens it). A node with no rows at all is the only clean-verdict output. Exit 0 regardless of content: findings are probabilistic, advisory by construction, and never drive exit codes (the deterministic sibling with exit-code semantics is `sm check`). `--json` emits `{ ok, kind: 'findings', findings[], total, fixedExcluded, staleExcluded }`, each entry mirroring the `state_findings` row (camelCase, including `model` and the `resolution*` fields when set) plus a derived `stale` boolean. Human mode renders the model name alongside the confidence when present. See [`db-schema.md` §state_findings](./db-schema.md). |
...
Actions are not invoked via `sm actions`. Probabilistic extensions (Actions AND finder Analyzers) queue via `sm job submit` (below); deterministic enrichment Actions (`io: ['network']` + an `enrichments/` report schema) execute via `sm refresh`; the remaining deterministic Actions dispatch in-process (`POST /api/actions/:id`, `sm bump`, ...).
...
| `sm job list [--status ...] [--extension ...] [--node ...]` | List jobs. `--extension` accepts a qualified `<plugin>/<ext>` id or a bare extension id (renamed from `--action` when Analyzers joined the queue). `--json` emits the job rows WITHOUT the `nonce` (the record credential travels only on `submit --json` / `claim --json`, see `job-lifecycle.md` §Atomic claim · Nonce exposure). |
...
| `GET /api/agent/install?provider=<id>` | implemented | Agent-drain-skill install probe for the SPA (the Settings → Project button; CLI counterpart: `sm agent status`, §Agent drain skill). Returns `{ provider, supported, skillDir, installed, stale }`; `supported: false` (with `skillDir: null`) when the Provider declares no `scaffold.skillDir`. Missing `provider` param → 400 `bad-query`; unknown provider id → 404 `not-found`. The `stale` field drives the button's three states (Install / Update / Up to date). |
