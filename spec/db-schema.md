...
- **Finder lane** (`origin = 'extension'`): when the job's extension is a probabilistic **Analyzer**, each entry of the validated report's `findings[]` array becomes one row, EXCEPT entries matching an active sidecar suppression on the node (`annotations.suppressions`, matched by the job's `extension_id` and, when the suppression narrows, the finding `type`; see [`schemas/annotations.schema.json`](./schemas/annotations.schema.json)): a suppressed finding is dropped before the write, so a user-dismissed judgment class never returns until the suppression is removed from the `.sm` file. The drop is announced ahead of the spend, not just applied at record: a finder submit over a node with a matching suppression emits a stderr advisory (see [`job-lifecycle.md` §Submit](./job-lifecycle.md#submit), suppressed-judgment advisory). `extension_id` / `extension_version` mirror the job's columns; per-row `confidence` is the finding's own value when present, else the report-level `confidence`; `model` mirrors the recording agent's self-reported `--model` (NULL when undeclared); `body_hash_at_generation` captures the node's `scan_nodes.body_hash` at record time; `job_id` records provenance.
