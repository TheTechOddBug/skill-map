...
| `score-phase-confidence` | Drop-in analyzer declaring `phase: 'score'` composes a confidence adjustment (`delta -0.4`, then a no-op `floor 0.5`) on top of the built-in `core/score-resolution` baseline (`set 1.0` for the resolved link); the folded, clamped `scan_links.confidence` lands at exactly `0.6`. |
