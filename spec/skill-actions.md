...
One config key governs the OFFERING: `skillActions.enabled`, boolean, default `false` ([`schemas/project-config.schema.json`](./schemas/project-config.schema.json)); the offering is OPT-IN (user decision 2026-08-09): installing a skill surfaces nothing until the operator turns the toggle on in Settings > Project. When `false` (the default), the `skills` bucket of `GET /api/nodes/:pathB64/prob-extensions` emits `[]` and a `skill:` submit refuses not-found; nothing else changes (discovery still runs at boot, membership stays boot-frozen either way, already-queued skill jobs still claim and record). The key is **project-local only** (the catalog is per-machine state, so offering it is a per-operator decision), written by the Settings > Project toggle through `PATCH /api/project-preferences` with no confirm gate, and read FRESH on every request, so flipping it takes effect immediately with no restart. Like every project-local-only key, its value is consent-gated per copy of the project: a hand-authored `settings.local.json` entry is ignored with a warning until `sm config set skillActions.enabled true` (or the Settings toggle) records the grant. This is deliberately an offering toggle, not per-skill enablement: uninstalling a skill remains the per-skill off switch (§What skill actions are NOT).
