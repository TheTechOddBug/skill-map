...
**Tags write-through.** (2026-07-21, the taxonomy leg of the summarizer split.) When `--status completed` and the recorded job's Action is a TAGGER, the record path merges the report's `tags[]` into the node's sidecar `annotations.tags` AFTER the record transaction commits (a filesystem write cannot share the DB transaction; the report itself already landed, so a failed apply loses no judgment). The tagger signal mirrors the summarizer's: an Action is a tagger iff its `report.schema.json` extends a schema under the canonical tags namespace ([`schemas/tags/`](./schemas/tags/), via `$ref`; today the single `markdown.schema.json` shape). Merge semantics: UNION with the existing `annotations.tags`, case-insensitive dedup, existing entries kept first in their order, new tags appended; a brand-new sidecar sources its required `identity` block from the live scan node (same rule as the dismiss write). The write rides the gated `.sm` channel and honours the STANDING `allowEditSmFiles` consent only (never an interactive prompt inside a record callback): without the grant the tags are NOT applied, a human-mode stderr advisory says so, and the record's exit code is untouched (the report still carries the tags for a later manual apply). On success the write-through `scan_nodes.annotations_json` mirror refreshes, so read surfaces see the new tags before the next scan. Storage-rule carve-out ([`architecture.md` §Storage rule](./architecture.md)): the applied tags land in the curation store because the operator's LAUNCH of the tagger is delegated curation (the same rationale that lets fixers edit bodies); once applied they are ordinary human-owned annotations.
