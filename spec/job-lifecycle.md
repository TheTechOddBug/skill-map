...
- **The resolution.** The fixer's report carries `resolved[]`, one entry per injected finding it considered: the finding's `id`, the `state` it puts the finding in (`fixed` = the fixer edited the file to resolve it; `declined` = it did not, the fix needs the author's decision), and a one-line `note`. At record the kernel stamps that state onto the matching `state_findings` rows (`resolution` / `resolution_note` / `resolution_by` / `resolution_at`), scoped to the job's node and the fixer's `analyzerIds`. `fixed` is a lifecycle state, not a closure: the row persists (hidden from the default `sm findings` view, re-checkable), and only the finder re-judging the current body deletes or reopens it; `declined` stays the author's visible TODO. See [`db-schema.md` §state_findings](./db-schema.md#state_findings).
