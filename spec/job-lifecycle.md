...
### Agent doorbell (wake-on-submit)

The inverse primitive: instead of an agent PARKED waiting for work (the blocking claim above, or the MCP `claim_job` `wait`), the server WAKES an agent when work arrives, so no session sits resident at all. Opt-in, off by default, one supported runtime today (OpenCode, the only target whose runtime exposes a local HTTP API a session can be started through).

- **Registration.** The runtime side registers a wake endpoint with the running server: `POST /api/agent/doorbell` with `{ url }` (same session-token header + loopback source gate as `POST /api/activity`). The URL MUST resolve to a loopback host; a non-loopback registration is refused (422). Registration is boot-scoped and in-memory: it dies with the server, re-registration is idempotent (last write wins), and nothing is persisted. OpenCode's generated activity plugin registers at plugin load (its input exposes the runtime's own `serverUrl`) and refreshes the registration by stamping the same URL onto every activity ingest body (`agentEndpoint`, optional), so a server restarted mid-session relearns the endpoint from ordinary traffic.
- **The wake.** When a `job.submitted` envelope crosses the server's broadcast choke point and the gate is on: wait a short settle delay, re-read the job, and only if it is STILL `queued` (nobody claimed it meanwhile) call the registered runtime API to create a fresh session (titled so the operator can spot it) and fire a single async prompt instructing it to run the `sm-process-jobs` skill in `once` mode: drain the queue, record each job, stop. The prompt call is fire-and-forget; the wake never blocks or fails the submit that triggered it.
- **Once, not resident.** The woken session drains and terminates. The doorbell replaces parking entirely on this runtime: idle cost is zero because nothing waits.
- **Cooldown, not a queue.** At most one wake per cooldown window (60s): a burst of submits produces one session, whose drain loop absorbs the burst (claim-until-empty). A submit landing while a previous drain is still running MAY produce a second session; the atomic claim makes that safe (one wins each job, the loser sees an empty queue and stops). Losing a wake is impossible by construction: every submit re-checks the queue, and a job nobody woke for is still claimable by every other path.
- **The boot ping never wakes.** `core/ai-ping-action` submits (the presence probe `sm serve` fires at startup) are excluded: the ping exists to test whether an ALREADY-parked agent answers, and waking an agent to answer it would burn tokens on every server restart to learn nothing.
- **Consent.** The gate is `jobs.wakeOnSubmit` (project-local only, default `false`): an autonomous agent spending the operator's tokens is a per-operator decision that must never travel via the shared repo. The toggle takes effect live (read at wake time, no server restart).
