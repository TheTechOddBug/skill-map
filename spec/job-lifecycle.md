...
**Suppressed-judgment advisory (finders).** When the submitted extension is an Analyzer and the target node's LIVE `.sm` sidecar carries `annotations.suppressions` entries matching the extension id (a standing `sm findings dismiss`, see [`cli-contract.md`](./cli-contract.md)), the CLI queues the job anyway but emits a human-mode stderr advisory naming the suppressed finding `type`s (an entry with no `type` suppresses the finder's every type and is reported as such). An advisory, never a refusal, for two reasons: the kernel safety lane is never suppressed, so the run still screens the node for injection; and a finder may emit types the suppression does not cover. What the advisory buys is honesty BEFORE the spend: the operator learns that the record path will drop the matching findings (the finder-lane suppression filter, [`db-schema.md` §state_findings](./db-schema.md#state_findings)) while the agent pass can still be reconsidered. Fires per node under `--all`; `--json` mode omits it (the stdout contract is unchanged, same posture as §Supersede).
