...
> = {
  ...
  opencode: { configPath: '.opencode/plugin/skill-map-activity.js', events: 0, shellOptIn: true },
};
...
export class StaticDataSource implements IDataSourcePort {
  ...
  async clearSessionJournal(): Promise<void> {
    throw new DataSourceError(
      'demo-readonly',
      'Clearing the session journal is not available in demo mode (static bundle is immutable).',
    );
  }
