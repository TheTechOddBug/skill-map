...
> **Step vs Phase glossary**: a **Step** (e.g. `Step 9`, `Step 14.4.b`) is an atomic feature milestone, one PR or a tightly-related sequence. A **Phase** (e.g. `Phase A`, `Phase B`, `Phase C`) is a multi-Step release target. Phase A = `v0.5.0` → `v0.6.0` (deterministic kernel + CLI + Web UI), Phase B = the stabilization run that ships **Beta** (`v0.52.0`: multi-runtime lens, Signal IR, annotations, extensible inspector, fused workspace, hardening), Phase C = real-time exploration (watch execution as it happens; shipped, the milestone before the LLM layer), Phase D = `v0.8.0` (job subsystem + findings), Phase E = `v1.0.0` (surface + distribution). Execution prose mixes both: `Step 14 ships v0.6.0 inside Phase A` is correct shorthand.
...
○       Execution snapshot           persistent immutable audit of every run (deferred, rides the same pipe later)
  ────────────────────────────────────────────────────────────────────────
   ▶ YOU ARE HERE · Step 10 COMPLETE (pull-only queue: skill-map never invokes the agent · claim/record processing + sm-process-jobs skill + Settings install · universal ai-summarizer-action · github/enrichment provenance verify) · skill-map@0.88/rc · next: Step 11 (findings pipeline + finder analyzers; the LLM-verb set died with Decision #137) or Step 16 (UI read-only LLM surfaces)
...
○  11b  Findings + finder analyzers  state_findings + findings/report envelope · sm findings reader · analyzers join the queue (extension_id rename) · per-node finders (consistency · redundancy · quality) · fixers via Modelo B (LLM verbs dropped, Decision #137)
...
○  17   UI: LLM surfaces v2          Semantic flows over queue + findings (describe · dedupe · cluster · impact · optimize, extension-driven per Decision #137) · queue inspector · findings management · cost dashboard · settings + plugins page · WCAG AA pass
...
The reference impl bundles built-ins for each kind: one Provider (`claude`), several Extractors (`slash`, `at-directive`, `backtick-mention`, `backtick-slash`, `backtick-dollar`, `markdown-link`, `backtick-path`), several Analyzers (`trigger-collisions`, `dangling-refs`, `link-kind-conflict`, `validate-all`), at least one Action, one Formatter (`ascii`). Hooks ship as needed for first-party integrations. `backtick-path` is the deliberate inverse of the code-strip policy: it extracts relative `.md` paths FROM code spans / fences (the Agent Skills standard's "load referenced files on demand" contract, the dominant reference shape in agent-authored skills) as `points` edges (Decision #127), normative in `spec/architecture.md` §Extractor · code-region file references. The trigger siblings `claude/backtick-mention`, `core/backtick-slash`, and `codex/backtick-dollar` are the second, narrower inversion (Decisions #134 / #135), one per trigger grammar so every provider lens is covered: bare `@handle` mentions, `/command` and `$skill` invocations inside code spans / fences become `mentions` / `invokes` links ONLY when they resolve to a real entity; the post-walk `prune-unresolved-code-triggers` gate silently drops the rest (npm scopes, decorators, shell paths and variables, at-rules), keyed on the occurrence-level `context` provenance, normative in `spec/architecture.md` §Extractor · code-region triggers. #135 also widened claude's mention matrix to the priority-ordered `['agent', 'skill', 'markdown']` (markdown nodes gained a `filename-basename` identifier) and exempted all-code-region self-loops from the `link-self-loop` warn (a doc showing its own usage is not a loop risk). The prose-side extractors additionally strip raw HTML (comments + tag tokens) via `stripCodeAndHtml`, so a link commented out as `<!-- [x](old.md) -->` or hiding in an attribute value never becomes a phantom edge; that HTML strip stays independent of the code-region inverse mask (HTML is not a code region).
...
`.skill-map/settings.json` is the canonical config for both the CLI and the bundled UI (the filename + `.local.json` partner mirror Claude Code). The loader deep-merges four layers, low → high precedence: **defaults** (compiled into the bundle) → **project** (`<cwd>/.skill-map/settings.json`, committed) → **project-local** (`settings.local.json`, gitignored, the only home for `PROJECT_LOCAL_ONLY_KEYS` like `allowEditSmFiles` / `scan.referencePaths` / `tutorialReminderDismissed`) → **env / flags**. There is **no user / global layer** (skill-map never reads `~/.skill-map/settings*.json`; per-machine preferences live in project-local). A malformed key warns and is skipped (the app never crashes on bad config); `--strict` makes it fatal. The layered loader is normative in [`spec/architecture.md`](./spec/architecture.md) §Config layering; the `sm ui --config <path>` escape hatch (Step 15) replaces the project + project-local layers with a single file.
...
The Decision #49 verb set (`sm what`, `sm dedupe`, `sm cluster-triggers`, `sm impact-of`, `sm recommend-optimization`) is DROPPED. Those verbs presupposed a runner ("submit one probabilistic job and render the result"); the pull-only decision removed the runner, so a verb can neither wait for nor render a probabilistic result, it can only read cache or enqueue, and both halves already exist generically (`sm show` renders summaries with the stale marker; `sm jobs submit` enqueues any probabilistic extension). Each capability ships as an extension instead: describe = `core/ai-summarizer-action` (shipped); consistency / redundancy / quality judgments = per-node finder Analyzers (`mode: 'probabilistic'`, findings via the canonical `findings/report.schema.json` envelope); refactor suggestions = the `skill-optimizer` Action queued generically; dedupe / trigger-clustering = cross-node finders pending the graph-scoped-job design (Step 11 second wave). The single surviving verb is **`sm findings`**, a generic reader of the kernel-owned `state_findings` table (the probabilistic sibling of `sm check`), plus the kernel-derived safety findings (`injection-detected` / `content-suspicious` / `content-malformed`) synthesized from any probabilistic report's `safety` block. Fixers are Actions joined via Modelo B (`precondition.analyzerIds`); a prose fixer is a queued job whose rendered prompt instructs the processing agent to edit the file with its own tools, skill-map never writes node bodies.
...
- ✅ **Hardening + telemetry** (Phase B). Loopback-only `sm serve`, node render caps, and opt-in, off-by-default usage / error telemetry: three independent toggles (`usageCliEnabled` / `usageUiEnabled` / `errorsEnabled`), anonymous PostHog usage + Sentry errors behind one shared first-run consent prompt, a shared anonymous install id as the `distinct_id`. Normative contract in [`spec/telemetry.md`](./spec/telemetry.md).
...
- ✅ **10**, Job subsystem + first probabilistic extension (Extractors are deterministic-only). Phases 0–F ✅ shipped on the rc channel: DB-only queue (`state_jobs` + content-addressed `state_job_contents`), the full `sm jobs` verb family (submit / list / show / preview `--last` / claim / cancel / fail / status / prune), `sm record` + nonce auth (its `--json` emits the synthetic `r-ext-` event envelope), the pull-only agent process (claim reaps first, stamps `runner=agent`; the short-lived `sm jobs run`/`RunnerPort` spawn path was built and then REMOVED by the 2026-07-13 pull-only decision), `ai-summarizer-action` (the first probabilistic Action, now the universal summarizer; the per-kind roster was dropped 2026-07-13) with the `state_summaries` write-through, and the `preamble-bitwise-match` conformance case. The E/F leftovers (`sm doctor`, seven checks after the runner probe died with the pull-only cut, and `sm actions list / show`) closed 2026-07-13, and Phase G closed the same week (agent process skill + HTTP/UI face, extension-mode-routing conformance, synthetic event envelope, `github/enrichment`). **Step 10 COMPLETE.**
- 🔮 **11**, Findings pipeline + finder analyzers (the LLM-verb set died with Decision #137; semantic capabilities ship as extensions, `sm findings` is the only surviving verb). Next after Step 10 closes; spec landed 2026-07-14 ahead of implementation.
- 🔮 **16**, Web UI: LLM surfaces v1 (initial). Render the probabilistic outputs Steps 10–11 emit, replaces the "Available in v0.8.0" empty-state placeholders shipped in 14.3 inspector with read-only surfaces for `state_summaries` / `state_enrichments` / `state_findings`. UI does not orchestrate jobs at this stage.
...
- 🔮 **17**, Web UI: LLM surfaces v2 (deeper). Promote the semantic capabilities into interactive UI flows (describe, dedupe, trigger-clustering, impact, optimization become panels / wizards driven by the finder / summarizer / fixer extensions per Decision #137; there are no per-capability CLI verbs to mirror). Job orchestration surface (queue inspector, retries, cancellations) is part of this Step. The queue-aware `/mcp` surface (typed claim/record tools over `sm serve`) lands in this arc too.
...
Continuation of wave 2 under Decision #137 (semantic capabilities are extensions, not verbs; the Decision #49 verb set is dropped, see §Semantic capabilities are extensions). Two waves:
...
**Wave 2, cross-node (needs design):** graph-scoped jobs (today's queue is strictly per-node: `extension + node + contentHash`). Unlocks dedupe and trigger-clustering finders. `/skill-map:explore` meta-skill also lands in this arc.
...
- **Verb panels**, reconciled + CLOSED 2026-07-19 against Decision #49 (the CLI verb set `sm what` / `dedupe` / `cluster-triggers` / `impact-of` / `recommend-optimization` was DROPPED under pull-only, each capability is an extension, not a verb). NOTHING ships as a standalone panel here:
  ...
  - the other four are EXTENSION-backed and tracked in the pending-plugins / extensions review, not as verb-panels: `what` (node summary) is already served by the shipped `core/ai-summarizer-action` + `state_summaries` (a presentation + on-demand re-run surface, not a new capability); `recommend-optimization` needs a new `skill-optimizer` Action (not yet built); `dedupe` + `cluster-triggers` are cross-node finders blocked on the wave-2 graph-scoped-job design (today's queue is strictly per-node: `extension + node + contentHash`).
...
- **A11y pass**, full WCAG AA pass for the verb flows (live regions for job status updates, focus trapping in dialogs, keyboard shortcuts for the queue inspector). Lighter passes were enough at 14.x; verb flows are interaction-heavy and warrant the audit. **Down payment (2026-07-20):** the graph map became keyboard-navigable via Foblex Flow 19's opt-in a11y layer (Tab into the canvas, arrow-key spatial navigation, Ctrl+arrow topology walk, Home/End jumps, Space+arrow node movement, +/- zoom; connection-creation and delete stay unbound on the read-only map), with selection reworked to single-owner (Foblex's selection drives the inspector / highlight state through one `applySelection` / `fSelectionChange` bridge). Shipped together with the v19 migration to the unified `fConnector` model (plain node ids, connection-level `fSourceId` / `fTargetId` sides); pinned at 19.1.2 (19.1.4 sits inside the 72h npm cooldown at upgrade time). The full verb-flow WCAG audit remains open, and was SCHEDULED LAST on 2026-07-21 (backlog review, user call): it runs as pre-RC polish, pegged right before the push / first RC, because the workbench surfaces are still reshaping week to week and an earlier audit would re-audit; the `app-a11y` agent drives the pass when picked up.
...
- **Tutorial reminder (topbar)**, a centered one-line nudge in the shell topbar pointing first-time users at `sm tutorial` (rendered as a monospace command chip) with a dismiss button. The dismissal persists to the project-local `tutorialReminderDismissed` flag (`settings.local.json`, gitignored, per-checkout) through the `project-preferences` BFF route (read at boot via `getProjectPreferences`, written on dismiss via `setProjectPreferences`), so unlike a localStorage flag it survives a browser-storage wipe and never shows again on that checkout; reset with `sm config reset tutorialReminderDismissed`. Suppressed in demo mode (the static bundle cannot run the CLI, and the demo banner already carries the install nudge).
...
- **MCP inspector-card enrichment (remaining cosmetic follow-up of the shipped MCP iteration)**. The functional MCP iteration SHIPPED across 2026-07 (dated log 2026-07-09/10/11); this entry is what is left. What shipped: **config-side discovery** (Phase 5b) as a declarative `mcpConfig` Provider capability, the provider names `{ path, dialect }` sources and the kernel (core) reads + parses them through the shared `kernel/util/mcp-config` util (FORMAT-only: JSON vs TOML), emitting the same virtual `mcp://<server>` node the consumer-side `core/mcp-tools` extractor draws from a node's `frontmatter.tools`, deduped with `derivedFrom` carrying both sources (codex `.codex/config.toml`, opencode `opencode.json`; Antigravity stays frontmatter-only, its MCP config is home-global at `~/.gemini/config/mcp_config.json`, off-limits to the project-local scanner); **live MCP-tool-invocation** on all four runtimes via the `activity` capability (claude / codex share `mapMcpInvocation` on the `mcp__<server>__<tool>` identifier, antigravity reads a `call_mcp_tool` wrapper's `ServerName`, opencode splits `<server>_<tool>`), so a live call lights the very same node; and the opt-in read-only **MCP server** on `/mcp` ([`spec/mcp-server.md`](./spec/mcp-server.md)). Normative shape in [`spec/architecture.md` §Provider · MCP config discovery](./spec/architecture.md). **Still deferred (cosmetic only):** the inspector MCP-node card should render the config-side metadata (transport, url / command, args, declared tools) instead of the placeholder body the Phase 5 consumer-only cards carry, so a selected `mcp://<server>` node reads its wiring at a glance rather than only its edges.
- **Codex AGENTS.md hierarchical walker (remaining half of the former Phase 6b)**. Phase 6 (shipped 2026-05-19) onboarded OpenAI Codex as a first-class provider with a TOML parser and a classifier for `.codex/agents/*.toml`. **Codex skills landed 2026-06-25.** The earlier draft of this item assumed Codex skills lived in a proprietary `.codex/skills/<name>/SKILL.md` mirroring Claude's `.claude/skills/`; per the official docs (https://developers.openai.com/codex/skills) that was wrong, Codex reads skills from the OPEN `.agents/skills/<name>/SKILL.md` standard (scanned CWD → repo root), the same on-disk home `agent-skills` already owns. The codex provider now classifies them as `codex`/`skill` by composing the `agent-skills` open-standard pieces (read rule, kind, resolution; NOT the reserved-name catalog, since Codex `$`-invokes skills, see §Provider · reservedNames) over a **multi-rule `read` array** (`.toml` agents + `.md` skills, one parser each); the `read` schema gained array support to express the mixed format, and `invokes` → `skill` resolution means an agent's `$skill-name` invocation (Codex's `$` grammar, the codex `dollar-skill` extractor) now links to its skill. One piece is still deferred. **AGENTS.md hierarchical cascade.** Codex reads `AGENTS.md` at every level from project root down to CWD, concatenates them in depth order, applies per-level `AGENTS.override.md` shadow files, and caps the total at `project_doc_max_bytes` (default 32 KiB). Today each `AGENTS.md` is just another markdown node the `core/markdown` fallback claims; the hierarchy is invisible. Two modelling options: (a) keep each `AGENTS.md` as its own node and add `extends` / `overrides` edges between them so the user reads the cascade visually (simpler, ~1.5 hours, refects the source faithfully); (b) synthesise virtual `agents-md://<dir>/` nodes representing "the effective instruction set Codex sees when working in `<dir>`" with `derivedFrom: [list of source AGENTS.md]` (more ambitious, ~3-4 hours, exposes runtime semantics but the synthetic body has no real-file backing). Option (a) is the recommended starting point; (b) opens when concrete demand for runtime-view appears. Both options need to support `project_doc_fallback_filenames` (Codex can be configured to also read `CLAUDE.md`, `CONTRIBUTING.md`, etc.) and surface a warning when a level's concatenated payload exceeds the configured cap. Effort: ~1.5 hours for the simple cascade (edges between real nodes); ~half a day for the full package including override semantics, fallback-filenames opt-in, and tests.
