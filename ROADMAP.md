...
- First finder built-ins: ONE focused extension per judgment (user decision 2026-07-14, same posture as the summarizer-split direction; the "one consistency finder with categories" alternative was dropped): redundancy, contradiction, incoherence, contraindication, quality. Finders judge INDEPENDENTLY, no cross-sibling deferral in the prompts: the operator picks which finders run, so a borderline defect must stay visible under any enabled subset, and the same defect reported by several enabled finders is supported coexistence (each row signed by its `extension_id`), not duplication to suppress (user decision 2026-07-14). Cost control via cheap-filter + expensive-verifier where a deterministic proxy exists (gzip ratio, token thresholds; the Step 22 sketch). Prompts reviewed by the user before each finder ships. SHIPPED (all 2026-07-14, each experimental / disabled by default, prompts user-approved, report schema narrowing `findings[].type` to the finder's own const): `core/node-redundancy` (the first of the roster, covered by the `findings-contract` conformance pair), then the batch `core/node-contradiction` / `core/node-incoherence` / `core/node-contraindication` (same mold, no new conformance cases: the envelope contract is generic), CLOSING the wave-1 roster at four. `quality` is PARKED outside the step entirely (user call 2026-07-14: he will drive it himself later, do not schedule it; the v1 actionability-only scope sketch survives in conversation history).
