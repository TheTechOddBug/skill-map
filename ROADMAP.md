...
1. **Declarative `read` instead of hand-rolled `walk()`**. Provider manifests declare `read: { extensions, parser }` (e.g. `{ extensions: ['.md'], parser: 'frontmatter-yaml' }`). The kernel walker owns symlink handling (an in-tree link is always followed with cycle detection; a link whose real target escapes the scan roots is refused by default and followed only when the project-local `scan.followExternalSymlinks` key is set, the realpath-containment gate reinstated on 2026-07-05 for the clone-and-scan threat model, superseding the 2026-07-02 "dereference wherever it points" decision, and extended on 2026-08-01 to the live WATCHER, which had been left dereferencing by chokidar's default so a committed `docs/x -> ~/` armed inotify watches across the operator's home; expressed as the same containment gate rather than chokidar's blunt `followSymlinks: false`, because live updates behind an INTERNAL symlinked directory are the whole reason that backend is selectable), TOCTOU re-stat, ignore-filter consumption, prototype-pollution strip, and the `js-yaml` JSON_SCHEMA pin so every Provider inherits them by construction. Built-in parsers ship as a closed set inside the kernel (`frontmatter-yaml`, `plain`); user plugins cannot register their own. A Provider that needs non-standard discovery still implements `walk()` directly, it wins over `read` and accepts the duplication of audit defences.
