...
●  7    Robustness                   sm watch (chokidar default, parcel selectable) · link-kind-conflict analyzer · sm job prune · trigger normalization
...
○       Execution snapshot           persistent immutable audit of every run (deferred, rides the same pipe later)
  ────────────────────────────────────────────────────────────────────────
   ▶ YOU ARE HERE · Step 10 COMPLETE (pull-only queue: skill-map never invokes the agent · claim/record drain + sm-run-queue skill + Settings install · universal markdown-summarizer · github/enrichment provenance verify) · skill-map@0.88/rc · next: Step 11 (findings pipeline + finder analyzers; the LLM-verb set died with Decision #137) or Step 16 (UI read-only LLM surfaces)
...
●  10e  Phase G polish               sm-run-queue skill + sm agent verbs · HTTP + Settings install button · github/enrichment (Model A, io:[network] + allowNetworkActions) · extension-mode-routing · (MCP queue-aware deferred to the UI arc)
...
loop:
  ...
  2. [rendered content from the sm job claim --json response, with the nonce]
...
Each job carries a unique `nonce` (on `state_jobs.nonce`, handed back by `sm job claim --json`); `sm record` requires a matching `--id` + `--nonce`, so a forged callback cannot close someone else's pending dispatch.
...
Missing-content / mid-run-crash handling (a `state_jobs` row whose `content_hash` no longer resolves in `state_job_contents` → `failed: job-file-missing`, a DB-corruption-only state since submit and prune keep the two tables consistent; orphaned `state_job_contents` rows referenced by no job → collected by `sm job prune`; crash between claim and read → covered by auto-reap) is specified in [`spec/job-lifecycle.md`](./spec/job-lifecycle.md).
...
The Decision #49 verb set (`sm what`, `sm dedupe`, `sm cluster-triggers`, `sm impact-of`, `sm recommend-optimization`) is DROPPED. Those verbs presupposed a runner ("submit one probabilistic job and render the result"); the pull-only decision removed the runner, so a verb can neither wait for nor render a probabilistic result, it can only read cache or enqueue, and both halves already exist generically (`sm show` renders summaries with the stale marker; `sm job submit` enqueues any probabilistic extension). Each capability ships as an extension instead: describe = `core/markdown-summarizer` (shipped); consistency / redundancy / quality judgments = per-node finder Analyzers (`mode: 'probabilistic'`, findings via the canonical `findings/report.schema.json` envelope); refactor suggestions = the `skill-optimizer` Action queued generically; dedupe / trigger-clustering = cross-node finders pending the graph-scoped-job design (Step 11 second wave). The single surviving verb is **`sm findings`**, a generic reader of the kernel-owned `state_findings` table (the probabilistic sibling of `sm check`), plus the kernel-derived safety findings (`injection-detected` / `content-suspicious` / `content-malformed`) synthesized from any probabilistic report's `safety` block. Fixers are Actions joined via Modelo B (`precondition.analyzerIds`); a prose fixer is a queued job whose rendered prompt instructs the draining agent to edit the file with its own tools, skill-map never writes node bodies.
...
| `sm-run-queue` (skill, materialised by `sm agent install`) | Agent drain deliverable | Any agent runtime | Step 10 Phase G | Teaches an agent the drain protocol: loop `sm job claim` → [agent reasons over the rendered content] → `sm record`. Decided 2026-07-13: ships as a runtime-agnostic skill written into the lens's `scaffold.skillDir` (no npm package, no host glue); the queue-aware `/mcp` surface follows in the UI arc as the typed second wrapper over the same protocol. |
