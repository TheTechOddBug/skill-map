...
- ✅ **Tutorial coverage of live activity** (2026-07-05): new sm-tutorial part "Real time: watch your agent run" after the daily loop (internal order 3; `cli` and `extend` shifted to 4 and 5). Three chapters shared by claude/codex/antigravity/opencode with per-provider trigger deltas: wire the hook (`sm activity install` with its consent prompt), restart the agent session and watch the portfolio glow (entry fast-forwards to the campaign project via the `harness-connected` seed; the tutorial agent's own tool calls fire the hooks; hard-stop restart bridged by the tutorial's resume flow), and the opt-in conversation-capture consent (spawn arrow + conversation dialog demo on claude/codex only), closing with the active provider's known-gaps note mirrored from README §Real-time. The pure `agent-skills` lens has no runtime to hook, so the part shows in its menu but exits with a short note (deliberate: no per-provider menu machinery). A `realtime-hook` footprint plus a preflight `sm activity uninstall` guard keep start-over and part-entry resets from orphaning a wired hook.
