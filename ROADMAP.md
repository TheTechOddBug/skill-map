...
**Import trust (security boundary).** Dropping a folder is not consent to run it. A project-local plugin is discovered (manifest parsed, listed) but its code is NOT imported by the runtime verbs (`sm scan`, `sm serve`, ...) until the operator grants local trust with `sm plugins enable <id>` (a `config_plugins` DB override). The committed `settings.json` baseline cannot grant import trust, a cloned repo controls its own `settings.json`, so cloning-and-scanning never auto-executes the repo's plugins; the runtime emits a one-time "found but not loaded" notice instead. Built-ins and explicit `--plugin-dir` are exempt; `--no-plugins` skips discovery. The `sm plugins` management verbs still import to enumerate (running them is itself explicit opt-in). Normative in [`spec/architecture.md`](./spec/architecture.md) §Locality.
