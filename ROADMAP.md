...
| Concept | Description |
|---|---|
| **Provider** | Extension kind. Recognizes a platform (today's built-in catalog: `claude`, `codex` for Codex, `antigravity` (reuses the open-standard classifier since Google adopted it), `agent-skills` for the vendor-neutral `.agents/skills/<n>/SKILL.md` layout, plus the `core` fallback that owns `markdown`), classifies each file into its node kind, and declares its on-disk `kinds` catalog (per-kind frontmatter `schema` + `ui` presentation block). **Deterministic-only**. The retired `gemini` Provider was removed when Google sunset Gemini CLI on 2026-06-18 and replaced it with Antigravity (which reuses the open-standard `.agents/skills/` classifier via the `agent-skills` Provider, so under its lens those skills classify as `antigravity`/`skill`). |
...
- ✅ **Active-lens migration, Phases 1–6** (2026-05-19 → 2026-05-23), post-v0.6.0 deterministic polish that lands the multi-runtime story end-to-end:
  ...
  - **Codex body extractor (TOML `instructions` field)**: ✅ landed via `read.bodyField: 'instructions'` (Step 13); the codex provider is now beta (enabled by default, auto-detects `.codex/`).
...
- **Codex body extractor (TOML `instructions` field)**, ✅ landed. The codex provider parses the TOML envelope and classifies `.codex/agents/*.toml` into agent nodes; the `instructions: """..."""` block (the agent's markdown body) now flows through the link extractors via the declarative `read.bodyField: 'instructions'` knob: the kernel walker yields that frontmatter field as the node body so the universal extractors (markdown-link, backtick-path, external-url) plus the lens-gated `claude/at-directive` + `claude/slash-command` (precondition widened to `['claude', 'codex']`) run over it. This is the "body can live on a parsed-frontmatter field" approach rather than a dedicated `codex/body-extractor`, so it generalises to any future provider whose prompt lives in frontmatter (e.g. a Gemini `prompt` field). A Codex agent whose instructions reference another agent via `@handle`, `[link](path.md)`, or a backtick path now surfaces in the graph under `activeProvider=codex`, feature parity with Claude. The post-v1.0 follow-ups for Codex (Phase 6b in §Deferred) stay deferred: hierarchical AGENTS.md walker, `.codex/skills/`. Remaining Step 13 work: Copilot.
