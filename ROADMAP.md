...
**Last updated**: 2026-05-11 (added a new deferred-beyond-v1.0 entry — "Live agent conversation view" — capturing the goal of streaming the LLM job transcript turn-by-turn in the UI Jobs inspector, with a CLI mirror via `sm job tail --conversation`. Previous edit: migrated the hardcoded experimental / deprecated stability icons on graph cards into a new built-in `core/stability` extractor that emits to the `card.footer.right` slot; dropped the dead-code injection icon that shared the same wrapper. Earlier edit: disabling a plugin via `sm plugins disable` or `PATCH /api/plugins/:id` now eagerly purges its `scan_contributions` rows so the UI stops rendering the plugin's chips immediately, instead of waiting for the next scan's catalog sweep; new `StoragePort.contributions.purgeByPlugin(pluginId, extensionId?)` on the kernel surface; plugin-managed state in `state_plugin_kvs` / dedicated tables is preserved per `plugin-kv-api.md`). Dated edit history of this file lives in `CHANGELOG.md` §Document changelog.
