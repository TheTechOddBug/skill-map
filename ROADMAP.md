...
**Fixer queue hygiene (open, from live testing 2026-07-15):** two fixer jobs for the same `(node, fixer)` can sit queued at once, because the duplicate index keys on `content_hash` and a sibling fixer's edit changes the body, so the second submit is technically a different job. Semantically it is the same request twice, and the processing run wastes a pass (observed: a re-rendered `node-clarify` job whose findings were already resolved, closing with an empty `editsSummary`). Candidate: for fixers, a new queued job SUPERSEDES an existing queued one for the same `(extension, node)` rather than joining it. Needs a decision on cancel-vs-refuse semantics.
