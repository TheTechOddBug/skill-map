...
- **A11y pass**, full WCAG AA pass for the verb flows (live regions for job status updates, focus trapping in dialogs, keyboard shortcuts for the queue inspector). Lighter passes were enough at 14.x; verb flows are interaction-heavy and warrant the audit. **Down payment (2026-07-20):** the graph map became keyboard-navigable via Foblex Flow 19's opt-in a11y layer (Tab into the canvas, arrow-key spatial navigation, Ctrl+arrow topology walk, Home/End jumps, Space+arrow node movement, +/- zoom; connection-creation and delete stay unbound on the read-only map), with selection reworked to single-owner (Foblex's selection drives the inspector / highlight state through one `applySelection` / `fSelectionChange` bridge). Shipped together with the v19 migration to the unified `fConnector` model (plain node ids, connection-level `fSourceId` / `fTargetId` sides); pinned at 19.1.2 (19.1.4 sits inside the 72h npm cooldown at upgrade time). The full verb-flow WCAG audit remains open.
