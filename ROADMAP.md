...
**Conformance cases deferred**: `preamble-bitwise-match` ✅ landed at Step 10 (Phase B closure): a `markdown-summarizer` job submitted via the new `setup.priorInvokes` staging phase, read back with `sm jobs preview --last`, asserted byte-exact against `spec/conformance/fixtures/preamble-v1.txt` via the new `stdout-contains-verbatim` assertion.
...
- `state_findings` table + write-through at `sm record` (finder lane `origin='extension'`; kernel safety lane `origin='kernel'` deriving `injection-detected` / `content-suspicious` / `content-malformed` from ANY probabilistic report's `safety` block, which makes `sm findings` useful from day one, `markdown-summarizer` already emits safety data).
...
- Fixers via Modelo B: `precondition.analyzerIds` joins fixer Actions to finders; prose fixes execute in the processing agent's own session (no new write surface, see `spec/architecture.md` §Modelo B). Findings injection (user decision 2026-07-14): so the fix acts on exactly what the finder reported, the kernel injects the node's matching findings into the fixer's rendered content (`## Findings to resolve` section, folded into `promptTemplateHash` like the report contract, zero re-key for non-fixer jobs; a fixer over a node with no such findings at all is refused). Stale rows ride the injection flagged `"stale": true` rather than being filtered (refined 2026-07-15, live testing): staleness is node-level, so fixing one section stales every finding on the node including untouched sections' still-present defects, and excluding them discarded valid judgments and forced a re-detection between every fix; the agent verifies each flagged entry against the current body and declines what no longer applies. Roster grouped by EDIT STRATEGY (`analyzerIds` is an array, so a fixer CAN serve several finders, though today each serves one): `core/node-consolidate` (redundancy, collapse repetition), `core/node-reconcile` (contradiction, resolve conflicting / jointly-risky directive pairs), `core/node-clarify` (incoherence, fill dangling refs / align terminology). Each experimental / disabled, prompt user-reviewed. All three now ship: `node-consolidate` (paired with the validated redundancy finder), then `node-reconcile` (serving the contradiction finder) and `node-clarify` (serving the incoherence finder), each with its report extending `report-base` (execution-only `resolved` + `editsSummary`, never the findings envelope).

**Fixer queue hygiene (landed 2026-07-16, supersede):** two fixer jobs for the same `(node, fixer)` could sit queued at once, because the duplicate index keys on `content_hash` and a sibling fixer's edit changes the body, so the second submit was technically a different job. Semantically it is the same request twice, and the processing run wasted a pass (observed: a re-rendered `node-clarify` job whose findings were already resolved, closing with an empty `editsSummary`). Resolution (user decision 2026-07-16, "reemplaza"): a fixer submit that finds an ACTIVE queued sibling for the same `(extension, node)` with a DIFFERENT `contentHash` CANCELS it and enqueues the new job in one transaction, reporting the superseded id on stderr. Two boundaries: an identical hash keeps the plain exit-3 duplicate refusal, and a running job is never superseded (an agent holds its claim). Fixer submits only; non-fixer jobs keep plain duplicate detection (`spec/job-lifecycle.md` §Findings injection for fixers · Supersede).
...
- `state_summaries` stays exercised by the universal `markdown-summarizer` (landed at Step 10; plugin-shipped summarizers join via the report-schema convention). `state_enrichments` accepts additional providers beyond `github-enrichment` when they ship, against the stable contract.
