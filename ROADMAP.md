...
○       Execution snapshot           persistent immutable audit of every run (deferred, rides the same pipe later)
  ────────────────────────────────────────────────────────────────────────
   ▶ YOU ARE HERE · real-time exploration shipped · skill-map@0.87 · next: the LLM optional layer (Phase D), opening with Step 10 (job subsystem)
...
- **MCP config-side discovery (Phase 5b of the active-lens migration)**. **(In active implementation from 2026-07-09 as the client / observer half of a broader MCP iteration; no longer purely deferred. Spec landed: [`spec/architecture.md` §Provider · MCP config discovery](./spec/architecture.md) + [`spec/mcp-server.md`](./spec/mcp-server.md).)** Phase 5 (shipped 2026-05-19) materialises MCP server nodes from the *consumer* side: the `core/mcp-tools` extractor scans `frontmatter.tools` on every skill / agent / command, picks up `mcp__<server>__<tool>` entries, and emits one virtual `mcp://<server>` node + a `references` link per unique server. The graph today shows MCPs only when at least one node uses them; declared-but-unused servers stay invisible, and used-but-undeclared servers materialise without any provenance back to where they should have been declared. Phase 5b closes the loop by reading the authoritative config files: Claude `settings.json` `mcpServers`, Cursor `.cursor/mcp.json`, OpenAI Codex `~/.codex/config.toml` + per-project `.codex/config.toml`, Gemini inline `mcp_servers` on subagents. Each per-provider extractor emits the same `mcp://<server>` virtual node with richer metadata (transport, command, args, declared `tools_provided`) and `derivedFrom: [<config path>]`. First-wins dedup in the orchestrator keeps the config-side node canonical when both sides emit the same id; the consumer-side stays as fallback for references that have no declaration. With both halves wired, three states become distinguishable: (a) declared + used → solid node + edges; (b) declared + unused → orphan node, surfaces as a hint that an MCP is set up but no agent uses it; (c) referenced + undeclared → `broken-ref` warning under sabor A, the operator sees they need to add it to the config. Architectural decision RESOLVED (2026-07-09, normative shape in [`spec/architecture.md` §Provider · MCP config discovery](./spec/architecture.md)): NOT a per-provider extractor and NOT a raw `ctx.readFile`, but a declarative `mcpConfig` Provider capability, the provider declares `{ path, dialect }` sources and the kernel (core) reads + parses them with the shared `kernel/util/mcp` util (the single owner of every MCP grammar and the `mcp://<server>` scheme), then emits the virtual nodes with metadata on the open `frontmatter` (no `node.schema.json` change). This keeps "provider owns its filesystem territory" while the parsing stays in core so a new vendor onboards by naming a file + dialect, never reimplementing a grammar. The `~/.codex/config.toml` reader extends the documented closed list of `os.homedir()` callers per `AGENTS.md` § Skill-map MUST NEVER read `$HOME` by default. Two siblings ship in the same iteration: **live MCP-tool-invocation detection** (deterministic off the runtime tool-use hook via the existing `activity` capability, `mcp__server__tool` being the same identifier the static extractor already parses, so a live call lights the very same `mcp://` node + edge, and catches used-but-undeclared servers at runtime), and a **read-only MCP server** (skill-map AS an opt-in, experimental server on `/mcp`, [`spec/mcp-server.md`](./spec/mcp-server.md)). UI follow-up: render MCP cards with the config-side metadata (transport, command, tools list) instead of the placeholder body the Phase 5 cards carry today.
