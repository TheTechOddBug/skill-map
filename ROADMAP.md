...
**Last updated**: 2026-05-07 (Step 9.6 review queue R14 closed — `loadPluginRuntime` honours BFF `runtimeContext` override, populated annotations endpoint test boots through the real composition root; only R6 (YAML serializer round-trip) remains open).
...
- `spec/schemas/annotations.schema.json` — catalog of 25 conventional fields. The load-bearing core is `version` (single integer monotonic, orthogonal to `stability`), `stability`, `supersedes`, `supersededBy`, `requires`, `conflictsWith`, `provides`, `related`. Provenance: `type`, `author`, `authors`, `license`, `source`, `sourceVersion`. Lifecycle dates: `created`, `updated`, `released`. Taxonomy: `tags`, `category`, `keywords`. Display: `icon`, `color`, `priority`, `hidden`. Docs: `docsUrl`. All optional; an empty `annotations: {}` is valid. Additional fields ride on `additionalProperties: true`; the built-in `unknown-field` rule warns on truly unrecognized keys (typo guard). Plugins that want first-class custom keys with their own validation declare `annotationContributions` in their manifest (Step 9.6.6).
...
- **R6 — YAML serializer drops comments + key order on round-trip.** ⏸ **Open by design, deferred 2026-05-07.** Resolution: keep `js-yaml`, document the contract loudly. Three mitigations shipped in the same closure pass: (a) `sm sidecar annotate` scaffold prints a banner explaining the round-trip contract on first creation (banner itself is dropped on first bump — by then the user has read it); (b) `spec/cli-contract.md` now carries a worked before/after example showing exactly what survives a bump; (c) `audit.bumpReason` is the canonical per-bump WHY field (per the B1 fix, it clears on a bump without `--reason` so it's truly per-bump, never historical). Reopen R6 — and apply the `js-yaml` → `yaml` (eemeli) swap with a `Document`-aware merge helper — when the first user surfaces a comment-loss complaint. Until then, the contract `bump rewrites the file; narrative goes in the .md body, per-bump rationale in audit.bumpReason` is the documented surface.
