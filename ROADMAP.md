...
- Fixers via Modelo B: `precondition.analyzerIds` joins fixer Actions to finders; prose fixes execute in the draining agent's own session (no new write surface, see `spec/architecture.md` §Modelo B). Findings injection (user decision 2026-07-14): so the fix acts on exactly what the finder reported, the kernel injects the node's current non-stale matching findings into the fixer's rendered content (`## Findings to resolve` section, folded into `promptTemplateHash` like the report contract, zero re-key for non-fixer jobs; a fixer over a node with no such findings is refused). Roster grouped by EDIT STRATEGY, not 1:1 with finders (`analyzerIds` is an array, so one fixer can serve several finders): `core/node-consolidate` (redundancy, collapse repetition), `core/node-reconcile` (contradiction + contraindication, resolve conflicting / risky directive pairs), `core/node-clarify` (incoherence, fill dangling refs / align terminology). Each experimental / disabled, prompt user-reviewed. All three now ship: `node-consolidate` (paired with the validated redundancy finder), then `node-reconcile` (serving the contradiction + contraindication finders via its two-id `analyzerIds`) and `node-clarify` (serving the incoherence finder), each with its report extending `report-base` (execution-only `resolved` + `editsSummary`, never the findings envelope).
