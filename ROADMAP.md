...
First UI hand-off for the probabilistic layer. Steps 10 and 11 fill `state_summaries`, `state_enrichments`, and `state_findings`; this Step makes that data visible without re-architecting any view.
...
- **Findings page**, new `/findings` route: filterable list (by severity, extension id, type, node, mirroring the `sm findings` filter surface) with deep-link to inspector, mirroring the existing list-view shape. No bulk actions yet, that lives in Step 17.
...
- **Probabilistic values ON the node** (user requirement 2026-07-14: "ese valor lo quiero ver dentro del nodo", e.g. a quality percentage, plus a CONFIDENCE badge): kernel-owned BFF decoration of `/api/nodes` from `state_findings` / `state_summaries` (per-node findings counts by severity, a finder-reported score, judgment confidence), painted as node-card chips / badges. NOT via `emitContribution` (finders never run at scan time) and NOT a new catalog slot (the catalog is the PLUGIN surface); the exact precedent is the `isFavorite` decoration from `state_node_favorites`, and the visual sibling is `link.confidence` edge opacity (deterministic confidence on links; this is probabilistic confidence on nodes). Open design: the aggregation rule when a node accumulates multiple judgments (min over fresh findings vs latest report vs per-finder in the inspector with an aggregate on the card). Scan purity untouched: the BFF read is a reader, not a scan input.
