...
**Last updated**: 2026-05-07 (Step 9.6 loop closure — UI inspector tiering + R4 deterministic report base + R10 live-BFF Playwright harness shipped on top of catalog curation. Review queue R1–R14 closed; R6 stays ⏸ open-by-design; R15 (BFF doesn't ship full `.sm` root) flagged from the UI work).
...
- `spec/schemas/annotations.schema.json` — curated catalog of 15 conventional fields (trimmed from 31 on 2026-05-07 after UX review). Versioning + supersession: `version` (single integer monotonic, orthogonal to `stability`), `stability`, `supersedes`, `supersededBy`, `requires`, `conflictsWith`, `related`. Provenance: `authors`, `license`, `source`, `sourceVersion`. Lifecycle: `released`. Taxonomy: `tags`. Display: `hidden`. Docs: `docsUrl`. All optional; an empty `annotations: {}` is valid. Additional fields ride on `additionalProperties: true`; the built-in `unknown-field` rule warns on truly unrecognized keys (typo guard). Plugins that want first-class custom keys with their own validation declare `annotationContributions` in their manifest (Step 9.6.6).
...
- **R6 — YAML serializer drops comments + key order on round-trip.** ⏸ **Open by design, deferred 2026-05-07.** Resolution: keep `js-yaml`, document the contract loudly. Two mitigations shipped in the same closure pass: (a) `sm sidecar annotate` scaffold prints a banner explaining the round-trip contract on first creation (banner itself is dropped on first bump — by then the user has read it); (b) `spec/cli-contract.md` now carries a worked before/after example showing exactly what survives a bump. The 2026-05-07 catalog-curation pass rolled back the third mitigation (`audit.bumpReason`) because the per-bump-rationale surface duplicated narrative that belongs in the `.md` body. Reopen R6 — and apply the `js-yaml` → `yaml` (eemeli) swap with a `Document`-aware merge helper — when the first user surfaces a comment-loss complaint. Until then, the contract is "bump rewrites the file; narrative goes in the `.md` body, which is never touched".
...
- **R15 — BFF `Node.sidecar` overlay does not ship the parsed `.sm` root.** 🟡 **Open 2026-05-07** — surfaced by the UI inspector tiering pass. Today the BFF's `GET /api/nodes` and `GET /api/graph` envelopes serialize `node.sidecar` as `{ present, status, annotations }` only — the full parsed sidecar object (`for`, `audit`, `settings`, plugin `<plugin-id>:` namespaces) is dropped at the wire boundary. The new `<sm-inspector-debug-panel>`, `<sm-inspector-audit-panel>`, and `<sm-plugin-contributions>` components in the UI degrade gracefully (placeholders + an optional `ISidecarOverlay.root` that's never populated today) but cannot fully render until the BFF starts shipping the root. **Resolution path**: extend `ISidecarOverlayApi` (in `src/server/...` and `ui/src/models/api.ts`) with an optional `root` field carrying the parsed YAML object; populate it in the kernel-to-API serializer (the same place `annotations` is currently materialized); thread through to the UI's `INodeView.sidecar.root`. Schema-side: the canonical wire shape is already in `node.schema.json`'s `sidecar` overlay (R1 closure); it just needs `root` declared as an optional bag of `additionalProperties: true`. Bias: ship next to whatever BFF endpoint touches `Node` shape next; not urgent because the UI degrades cleanly today.

**Catalog curation 2026-05-07**: 16 annotations dropped from the catalog after UX review (orphan capability tokens, redundant taxonomy, social-profile fields out of skill-map domain). `audit.bumpReason` rolled back. The kept set is the load-bearing 15: `version`, `stability`, `supersedes`, `supersededBy`, `requires`, `conflictsWith`, `related`, `authors`, `license`, `source`, `sourceVersion`, `released`, `tags`, `hidden`, `docsUrl`. Dropped: `provides`, `type`, `author`, `created`, `updated`, `category`, `keywords`, `icon`, `color`, `priority`, `readme`, `examplesUrl`, `github`, `homepage`, `linkedin`, `twitter`. Cascade: `--reason` flag dropped from `sm bump`; `reason` field dropped from `POST /api/sidecar/bump` body; `audit.bumpReason` removed from `sidecar.schema.json`. `additionalProperties: true` stays everywhere so legacy / opaque keys still ride through; the `unknown-field` rule warns on any post-curation typo. R6's mitigation set updates: `audit.bumpReason` is no longer the canonical per-bump rationale surface — the contract is now "bump rewrites the file; narrative goes in the `.md` body, which is never touched". The deepMerge null-as-delete primitive in `kernel/sidecar/store.ts` is retained without a current caller (architecturally sound for future per-write erase semantics).
