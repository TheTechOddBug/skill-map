...
**Auto-fix chain (user requirement 2026-07-15, designed not built):** an OPTIONAL flow that chains finder to fixer. The relation already exists one-way (the fixer declares `precondition.analyzerIds`); auto-fix needs the cheap deterministic INVERSE lookup (given a finder, which Actions list it), the same join the inspector's "Recommended for issues" uses. Mechanism: a built-in Hook on `job.completed` filtered to finder jobs that calls `ctx.queue(<fixer>)`, the exact documented "deterministic Hook enqueues a probabilistic Action" pattern; it composes with pull-only because the hook fires inside `sm record`, so the processing agent's own loop continues into the fix in one session (claim finder, record, hook queues fixer, claim fixer, edit, record). OPTIONAL in BOTH surfaces (user requirement), which shipping it as a Hook PLUGIN satisfies for free: plugin enablement is already a dual toggle (`sm plugins enable core/auto-fix` on the CLI, the Settings plugins panel in the UI), so no bespoke `--auto-fix` flag or setting is invented. It ships DISABLED: enabling it means "edit my files before I read the findings", a loud opt-in for a tool whose posture is that the human decides. The manual half is symmetric and half-built: CLI `sm jobs submit <fixer> -n <node>` exists today, the UI equivalent is a per-finding "fix this" button (Step 16/17), the one to ship first. Rules: no findings, no queue; several matching fixers all queue; the `human-decision` state stays the safety net so anything needing author judgment is punted, never invented.
