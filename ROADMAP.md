...
### Step 9.6 — Annotation system (sidecar `.sm` files) — pending

> ⏸ **Pre-Step-10 prerequisite.** Closes the deferred decision from Step 9.5. Skill-map's annotation layer (today's `metadata` block plus `type` / `author` / `authors` / `license` — none of which Anthropic owns) lives in **co-located YAML sidecars with extension `.sm`** in the same directory as the markdown node they annotate. Architectural consensus reached in conversation 2026-05-05; sixteen conceptual decisions captured in `memory/project_annotation_architecture.md` and formally recorded in Decision #125. Closes before Step 10 starts so the `skill-summarizer` pipeline writes against a stable annotation surface — same gating rationale as Step 9.5.
...
- **Annotations catalog**: ~25 typed fields covering versioning, supersession, provenance, taxonomy, lifecycle, integration, display, documentation. Schema is `additionalProperties: true` so plugin / user extensions ride freely; Tier 1 `unknown-field` rule warns on truly unrecognized keys.
...
- **9.6.1 — Sidecar schema spec** ⏸. New `spec/schemas/sidecar.schema.json` (root shape with reserved blocks + `additionalProperties: true`) and `spec/schemas/annotations.schema.json` (~25-field catalog). Conformance fixture extended with sample `.sm`. `spec/index.json` regenerated. Changeset for `@skill-map/spec` (minor, greenfield-permitted breaking). No code changes — locks the shape.

- **9.6.2 — Kernel sidecar reader + drift detection** ⏸. Walker reads `<basename>.sm` next to each `<basename>.md` it finds. YAML parsing (`js-yaml` already on dependency tree). AJV validation against the new schemas. SHA-256 computation of body and frontmatter, comparison against `for.bodyHash` / `for.frontmatterHash`; drift → warning issue (built-in rule `annotation-stale`). Annotations denormalized into a new `state_annotations` table or extended `scan_nodes` columns (decision per how the kernel currently denormalizes `metadata.{stability, version, author}` — likely the latter for read parity). Tests: walk fixtures with `.sm` present and absent; drift cases; orphan `.sm` (no matching `.md`); malformed YAML; invalid schema content.

- **9.6.3 — Bump Action + sidecar write channel** ⏸. New runtime concept: actions can return `{ writes: [{ kind: 'sidecar', path, changes }] }` payloads; kernel materializes via a `SidecarStore` port (mirrors `StoragePort` shape but writes YAML files in repo). Built-in deterministic Action `bump` reads the node, computes new hashes, increments version, returns the payload. Action stays pure (testable, dry-runnable). `IAction` runtime contract extension is additive. Tests: bump on stale nodes, bump on already-current nodes (no-op or refusal — pick), atomicity under concurrent edits.

- **9.6.4 — CLI verbs** ⏸. `sm bump <node>` for single-node bump (gated by drift unless `--force`); `sm bump --pending [--staged]` for batch; `sm refresh <node>` to update hashes only without bumping version; `sm prune-annotations` to remove orphan `.sm` files (with `--dry-run`); `sm hooks install pre-commit-bump` to install the git pre-commit hook (chained with existing hooks if any); optional `sm annotate <node>` scaffold helper that creates an empty `.sm` ready for editing. CLI reference regenerated. Tests cover each verb plus integration with the `bump` Action.

- **9.6.5 — UI integration: stale + bump** ⏸. Card receives stale-badge surface (orange icon) when reader reports drift on the node. Inspector shows current annotations as readable surface. Bump button in inspector — disabled when no drift, dispatched as Action call when clicked, live-updates via WS. List view filter `--stale`. Tests: card render with/without drift; inspector bump button enable state; e2e bump flow.

- **9.6.6 — Plugin annotation contributions** ⏸. Manifest field `annotationContributions: { <key>: { schema: ..., ownership: 'exclusive' | 'shared', location: 'namespaced' | 'root' } }`. Default `location: 'namespaced'` (writes to `<plugin-id>:` block); `location: 'root'` requires `ownership: 'exclusive'` and is treated as elevated trust (documented requirement, plugin author guide explains). Validation at plugin load; conflict detection (two plugins claiming the same root key with `exclusive` → error at load). Kernel exposes runtime catalog `getRegisteredAnnotationKeys()` for UI autocomplete. Aligns with Decision #112 (output schema for plugin storage writes); same pattern, different write target. Tests: namespaced writes, root opt-in, collision detection, malformed contribution manifest.
