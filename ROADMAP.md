...
- First finder built-ins: ONE focused extension per judgment (user decision 2026-07-14, same posture as the summarizer-split direction; the "one consistency finder with categories" alternative was dropped): redundancy, contradiction, incoherence, contraindication, quality. Cost control via cheap-filter + expensive-verifier where a deterministic proxy exists (gzip ratio, token thresholds; the Step 22 sketch). Prompts reviewed by the user before each finder ships.
