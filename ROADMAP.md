...
- 🔮 **13**, Multi-host Providers (Codex body extractor; Copilot; generic). Codex itself + agent-skills + Antigravity already landed during the active-lens migration; the Codex body extractor (TOML `developer_instructions` field → markdown / at-directive / slash pipeline) landed via `read.bodyField`, leaving Copilot as the remaining piece. The legacy Gemini Provider shipped at Step 9.7 and was retired in 2026-05.
