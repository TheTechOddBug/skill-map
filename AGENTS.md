...
- **User-facing changesets get a `## User-facing` section** — when a `.changeset/*.md` that bumps `@skill-map/cli` introduces a change the end-user notices (new verb, UI feature, behavioural shift, breaking surface), append a `## User-facing` H2 section with a short user-focused note (markdown allowed: bold, links, code spans). The technical body above stays as-is for the auto-generated `CHANGELOG.md`; the `## User-facing` block is extracted by `scripts/build-user-changelog.js` (run as the first step of `release:version`) into `ui/src/data/user-changelog.json` and rendered in the Settings → Changelog tab. Internal-only changes (refactors, infra, fixes invisible at the surface) omit the section — releases where every changeset is internal are written out as a `kind: 'internal'` placeholder so the version still appears in the changelog with a "no user-facing changes this time" line.
