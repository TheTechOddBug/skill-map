...
- `src/server/options.ts`, `IServerOptions` + `validateServerOptions`. Loopback-only check for `--dev-cors`; port range `[0, 65535]`; rejects `--no-built-ins + watcher on` (would persist empty scans on every batch).
