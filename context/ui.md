...
- The glob is restricted to `.html` because extending it to `*.ts` makes Angular's CLI plugin double-process component sources and surface stale template-typecheck errors. **Inline-template `.ts` components are NOT covered by the guard** (no inline template currently declares a `target="_blank"`; `settings-about` was migrated to a sibling `.html` so the guard covers it). When adding a new external link inside an inline template, either migrate the template to a sibling `.html` file (preferred, the guard then picks it up automatically) or add a dedicated assertion alongside the component's own spec.
...
- **Markdown**: a remote image is the one markdown feature that issues a request purely by being rendered, so `ui/src/services/markdown-renderer.ts` overrides markdown-it's `image` renderer rule and an image token NEVER becomes an `<img>`. It becomes a **click-to-load placeholder** naming the image and the HOST the request would go to, so the operator consents with the destination in view; the fetch happens only on click. Two shapes, selected by the `smImageMode` flag threaded through markdown-it's `env` (one parser instance): `interactive` for block renders (`render` / `renderToHtml`), a `<button class="sm-md-img" data-sm-img-src="…">`; `static` for inline renders (`renderInline`), a bare `<span class="sm-md-img--static">` with no URL attribute, because the inline hosts (node cards, descriptions) already own click and drag. A `src` that fails `httpUrlOrNull` degrades to the static span in either mode. The click handler is the standalone `[smMarkdownImages]` directive (`ui/src/app/core/markdown-images.directive.ts`), applied to the BLOCK hosts only (inspector body, vendor initial prompt, conversation bubbles) because `[innerHTML]` output is inert; it re-validates the URL, then swaps in an `<img referrerpolicy="no-referrer">`. Stateless by design, a re-render restores the placeholder. `img` stays in `FORBID_TAGS` (plus `style`, `srcset`) as the backstop against a regression in that rule. Chip styling is global (`.sm-md-img*` in `ui/src/styles.css`, the markup carries no `_ngcontent` attribute). Covered by the `audit L-1` block in `ui/src/services/__tests__/markdown-renderer.spec.ts` and `ui/src/app/core/__tests__/markdown-images.directive.spec.ts`.
