...
(() => {
  ...
  const PHASES = [
    ...
    },
    {
      id: 'E',
      status: 'done',
      release: { en: 'skill-map@1.0.0', es: 'skill-map@1.0.0' },
      title: { en: 'Surface & distribution', es: 'Superficie y distribución' },
      sub: { en: 'Deeper UI on the AI layer, accessibility, single-package release.', es: 'UI más profunda sobre la capa AI, accesibilidad, release de paquete único.' },
      brief: {
        en: 'The product reaches 1.0 here. The AI layer from Phase D surfaces deeper in the UI: connected AI agents drive the job queue over MCP, a settings-hierarchy viewer, and a WCAG AA accessibility pass. Plus distribution polish: llms.txt for AI ingestion generated from the spec, release infrastructure, and @skill-map/cli as a single npm package with the UI bundled inside. One process, one port, one command.',
        es: 'Aquí el producto llega a 1.0. La capa AI de la Fase D aparece más profundo en la UI: agentes AI conectados manejan la cola de jobs vía MCP, un visor de la jerarquía de settings, y un pase de accesibilidad WCAG AA. Además, pulido de distribución: llms.txt para ingesta por LLMs generado desde la spec, infraestructura de release, y @skill-map/cli como un único paquete npm con la UI empaquetada adentro. Un proceso, un puerto, un comando.',
      },
      list: 'steps',
      items: [
        { id: '17',  status: 'done',    title: { en: 'UI: AI surfaces v2 (deeper)',     es: 'UI: superficies AI v2 (más profundo)' },    body: { en: 'Connected AI agents driving the job queue over MCP · settings-hierarchy viewer · WCAG AA accessibility pass',                     es: 'Agentes AI conectados manejando la cola de jobs vía MCP · visor de jerarquía de settings · pase de accesibilidad WCAG AA' } },
