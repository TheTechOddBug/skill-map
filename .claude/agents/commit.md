...
description: |
  ...
  rules in CONTRIBUTING.md), regenerate spec integrity hashes,
  ROADMAP cross-references, and the conventional commit itself. The
  workspace CHANGELOG.md files are generated by `changeset version`, this
  agent never hand-edits them. Never pushes, never amends, never skips
...
| `**/CHANGELOG.md` (any workspace) | — | — | NO — release notes, not a releasable change; `check-changeset.js` filters them out |
...
### 5. Do NOT hand-edit `spec/CHANGELOG.md` or `src/CHANGELOG.md`

These are GENERATED by `changeset version` at release time from the
changeset body you write in step 6. Never add entries by hand, that is
...
<ONE short paragraph>: what changed and where a reader notices it. This
text is published verbatim into `CHANGELOG.md`, so keep it terse and put
the deep why / how / which-files detail in the PR description. No tables,
no sub-headings, no sub-bullets, no multi-paragraph breakdown (a
pre-commit guard rejects them, see §6.1).
```
...
One short paragraph below covers all bumped packages. Do NOT split into
sub-bullets per package, the guard rejects them; fold the packages into
the single paragraph.

#### 6.1. Decide whether to add a `## User-facing` section

The changeset's technical body is ONE SHORT PARAGRAPH (what changed +
where a reader notices it). It feeds the auto-generated workspace
`CHANGELOG.md` verbatim, so no verbose breakdowns, tables, sub-bullets,
or file inventories, that depth goes in the PR description. A pre-commit
guard (`scripts/build-user-changelog.js --check`, run by the hook when a
`.changeset/*.md` is staged) rejects a body that has a table, a
sub-heading, a sub-bullet, more than one paragraph, or exceeds
`MAX_TECHNICAL_BODY_CHARS` (500). Same check fires in `release:version`.
...
<one short paragraph: what changed and where a reader notices it.
no tables, no sub-bullets, no sub-headings, no second paragraph.>
...
- **Doesn't edit code beyond what the user already changed.** This agent
  ...
  section when applicable per §6.1), the regenerated `spec/index.json`,
  and a cross-reference in `ROADMAP.md`. The workspace `CHANGELOG.md`
  files are NOT hand-written, `changeset version` generates them at
  release time. Anything else → ask the user first.
