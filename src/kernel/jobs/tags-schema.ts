 ...
 * (`spec/job-lifecycle.md` §Tags proposal), the exact mirror of the
 * summarizer detection in `summary-schema.ts`: an Action declares itself
 * a tagger through the output contract it already owns, its
 * `report.schema.json` extends a schema under the canonical tags
 * namespace (`spec/schemas/tags/`, today the single
 * `markdown.schema.json` shape) via `$ref`, typically inside `allOf`.
 * There is NO manifest flag; the record path infers the opt-in from the
 * schema the kernel already loads.
 *
 ...
 * surfaces its report's `tags[]` as a proposal on the `job.completed`
 * event (it writes no curation, the human saves it). Works on both
