...
import { validateAllAnalyzer } from './index.js';
...
describe('validate-all rule', () => {
  it('emits no issues on an empty graph', async () => {
    const issues = await validateAllAnalyzer.evaluate({ nodes: [], links: [], emitContribution: noopEmit });
  ...
  it('emits an error issue when a link has an invalid kind', async () => {
    ...
    strictEqual(issues[0]?.analyzerId, 'validate-all');
  ...
  it('emits an issue per malformed node', async () => {
    ...
    const issues = await validateAllAnalyzer.evaluate({ nodes: [bad], links: [], emitContribution: noopEmit });
    ok(issues.length >= 1);
    strictEqual(issues[0]?.analyzerId, 'validate-all');
  });
});
 ...
 * per-kind validator, the orchestrator hands `validate-all` a node
 * with a blank frontmatter. The analyzer catches that case so the
 * operator gets the same red-alert affordance as for `broken-ref`.
 */
describe('validate-all rule, frontmatter base check', () => {
  ...
  it('flags a node whose frontmatter has no `name`', async () => {
    ...
    const issues = await validateAllAnalyzer.evaluate({
      nodes: [node],
      links: [],
...
describe('validate-all rule, view contribution emission', () => {
  ...
  it('emits an alert + chip per node with at least one finding', async () => {
    ...
    await validateAllAnalyzer.evaluate({ nodes: [broken], links: [], emitContribution: emit });
  ...
  it('emits no contributions for a well-formed node', async () => {
    ...
    const { emit, log } = collectEmits();
    await validateAllAnalyzer.evaluate({ nodes: [ok], links: [], emitContribution: emit });
  ...
  it('aggregates per-node, one alert + chip even when two checks fire on the same node', async () => {
    ...
    const { emit, log } = collectEmits();
    await validateAllAnalyzer.evaluate({ nodes: [doubleBad], links: [], emitContribution: emit });
