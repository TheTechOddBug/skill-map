/**
 * CLI strings emitted by the `sm sidecars` sub-namespace
 * (`cli/commands/sidecar.ts`):
 *
 *   - `sm sidecars refresh <node-path>`, refresh `for.{bodyHash,
 ...
 *   - `sm sidecars prune [--dry-run]`, delete orphan `.sm` files
...
export const SIDECAR_TEXTS = {
  ...
  refreshNoSidecarHint:
    'Run `sm sidecars annotate` to scaffold one, or `sm bump` to create it via the Action.',
  ...
  refreshFailed: '{{glyph}}  sm sidecars refresh: {{message}}\n',
  ...
  pruneAborted: '{{glyph}}  sm sidecars prune: aborted by user. No files deleted.\n',
  ...
  pruneDeleteFailed:
    ...
    '   {{hint}}\n',
  pruneDeleteFailedHint: 'Continuing with the remaining files; re-run `sm sidecars prune` after fixing the underlying issue.',
  ...
  annotateFailed: '{{glyph}}  sm sidecars annotate: {{message}}\n',
