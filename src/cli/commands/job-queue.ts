...
export class JobSubmitCommand extends SmCommand {
   ...
   * set dedupes at emit) and as `$screen_name`. One entry per created job,
   * so a fully refused submit contributes nothing. See spec/telemetry.md.
   */
  private stashSubmitUsage(submittedCount: number, extensionId: string): void {
    if (submittedCount === 0) return;
    addInvocationExtensions(Array.from({ length: submittedCount }, () => extensionId));
    setInvocationScreenName(extensionId);
  }
  ...
  private async reportAll(
    ...
    this.stashSubmitUsage(submitted.length, prepared.extensionId);
