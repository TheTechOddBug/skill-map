...
function buildSubmit(): JobSubmitCommand {
...
async function runFullLoop(proj: IProject): Promise<string> {
  await withCwd(proj.root, async () => run(buildSubmit(), captureContext()));
