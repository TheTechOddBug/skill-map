...
describe('sm plugins doctor, disabled is not a failure', () => {
  it('exit 0 when the only non-loaded plugin is disabled', () => {
    ...
    // is 7, the disabled `mock-h` drop-in plus the six experimental
    ...
    // opt-in `core/auto-fix` hook, the universal summarizer
    // `core/ai-summarizer-action` (experimental until its Step 16 UI
    // surface lands), and the deterministic-analyzer fixer
    // `core/ai-reference-action` (fixes `core/reference-broken`,
    // experimental). The three probabilistic finders
    // (`core/ai-redundancy-analyzer` / `core/ai-contradiction-analyzer` /
    // `core/ai-incoherence-analyzer`) and the three finder-paired fixers
    ...
    // enabled.)
    assert.match(r.stdout, /disabled\s+7/);
  });
});
