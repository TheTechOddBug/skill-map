...
| Action | `core` | `node-consolidate` | FIRST fixer (experimental, ships disabled): a probabilistic Action declaring `precondition.analyzerIds: ['core/node-redundancy']`. At submit the kernel injects the node's current non-stale `node-redundancy` findings into a `## Findings to resolve` section (`spec/job-lifecycle.md` §Findings injection for fixers), and refuses when the node has none; the draining agent edits the file to collapse the flagged repetition. skill-map never writes the body. |
| Action | `core` | `node-reconcile` | Fixer (experimental, ships disabled) serving TWO finders: `precondition.analyzerIds: ['core/node-contradiction', 'core/node-contraindication']`. At submit the kernel injects the node's current non-stale contradiction + contraindication findings into a `## Findings to resolve` section, and refuses when the node has none; the draining agent edits the file to settle the conflicting or jointly-risky directive pairs, preserving every distinct requirement. skill-map never writes the body. |
| Action | `core` | `node-clarify` | Fixer (experimental, ships disabled) for `core/node-incoherence`: `precondition.analyzerIds: ['core/node-incoherence']`. At submit the kernel injects the node's current non-stale incoherence findings into a `## Findings to resolve` section, and refuses when the node has none; the draining agent edits the file to fix dangling references, align drifting terminology, and supply missing context, without inventing facts. skill-map never writes the body. |
