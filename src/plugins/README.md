...
| Rule | `core` | `node-contradiction` | Probabilistic finder (experimental, ships disabled): judges one node for internal contradictions (directive pairs that cannot both be followed, or whose combination is jointly risky); findings of type `contradiction`. Same job-loop shape as `node-redundancy`; finders judge independently. |
...
| Action | `core` | `node-reconcile` | Fixer (experimental, ships disabled) for `core/node-contradiction`: `precondition.analyzerIds: ['core/node-contradiction']`. At submit the kernel injects the node's contradiction findings into a `## Findings to resolve` section, stale ones included and flagged, and refuses only when the node has none at all; the processing agent edits the file to settle the conflicting or jointly-risky directive pairs, preserving every distinct requirement. skill-map never writes the body. |
