...
| Rule | `core` | `node-redundancy` | FIRST probabilistic finder (experimental, ships disabled): judges one node for internal redundancy via the job loop (`sm jobs submit` / `sm jobs claim` / `sm record`); findings of type `redundancy` land in `state_findings`, read with `sm findings`. No `evaluate()`, never runs at scan time. |
| Rule | `core` | `node-contradiction` | Probabilistic finder (experimental, ships disabled): judges one node for internal contradictions (directive pairs that cannot both be followed); findings of type `contradiction`. Same job-loop shape as `node-redundancy`; finders judge independently. |
| Rule | `core` | `node-incoherence` | Probabilistic finder (experimental, ships disabled): judges one node for internal incoherence (dangling references, drifting terminology, steps out of order); findings of type `incoherence`. Same job-loop shape as `node-redundancy`; finders judge independently. |
| Rule | `core` | `node-contraindication` | Probabilistic finder (experimental, ships disabled): judges one node for contraindications (individually valid directives that are jointly risky); findings of type `contraindication`. Same job-loop shape as `node-redundancy`; finders judge independently. |
| Action | `core` | `node-consolidate` | FIRST fixer (experimental, ships disabled): a probabilistic Action declaring `precondition.analyzerIds: ['core/node-redundancy']`. At submit the kernel injects the node's `node-redundancy` findings into a `## Findings to resolve` section (`spec/job-lifecycle.md` §Findings injection for fixers), stale ones included and flagged `"stale": true` for the agent to verify against the current body, and refuses only when the node has none at all; the processing agent edits the file to collapse the flagged repetition. skill-map never writes the body. |
| Action | `core` | `node-reconcile` | Fixer (experimental, ships disabled) serving TWO finders: `precondition.analyzerIds: ['core/node-contradiction', 'core/node-contraindication']`. At submit the kernel injects the node's contradiction + contraindication findings into a `## Findings to resolve` section, stale ones included and flagged, and refuses only when the node has none at all; the processing agent edits the file to settle the conflicting or jointly-risky directive pairs, preserving every distinct requirement. skill-map never writes the body. |
| Action | `core` | `node-clarify` | Fixer (experimental, ships disabled) for `core/node-incoherence`: `precondition.analyzerIds: ['core/node-incoherence']`. At submit the kernel injects the node's incoherence findings into a `## Findings to resolve` section, stale ones included and flagged, and refuses only when the node has none at all; the processing agent edits the file to fix dangling references, align drifting terminology, and supply missing context, without inventing facts. skill-map never writes the body. |
