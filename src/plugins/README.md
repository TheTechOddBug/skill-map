...
| Rule | `core` | `node-redundancy` | FIRST probabilistic finder (experimental, ships disabled): judges one node for internal redundancy via the job loop (`sm job submit` / `sm job claim` / `sm record`); findings of type `redundancy` land in `state_findings`, read with `sm findings`. No `evaluate()`, never runs at scan time. |
...
| Action | `core` | `node-consolidate` | FIRST fixer (experimental, ships disabled): a probabilistic Action declaring `precondition.analyzerIds: ['core/node-redundancy']`. At submit the kernel injects the node's `node-redundancy` findings into a `## Findings to resolve` section (`spec/job-lifecycle.md` §Findings injection for fixers), stale ones included and flagged `"stale": true` for the agent to verify against the current body, and refuses only when the node has none at all; the draining agent edits the file to collapse the flagged repetition. skill-map never writes the body. |
| Action | `core` | `node-reconcile` | Fixer (experimental, ships disabled) serving TWO finders: `precondition.analyzerIds: ['core/node-contradiction', 'core/node-contraindication']`. At submit the kernel injects the node's contradiction + contraindication findings into a `## Findings to resolve` section, stale ones included and flagged, and refuses only when the node has none at all; the draining agent edits the file to settle the conflicting or jointly-risky directive pairs, preserving every distinct requirement. skill-map never writes the body. |
| Action | `core` | `node-clarify` | Fixer (experimental, ships disabled) for `core/node-incoherence`: `precondition.analyzerIds: ['core/node-incoherence']`. At submit the kernel injects the node's incoherence findings into a `## Findings to resolve` section, stale ones included and flagged, and refuses only when the node has none at all; the draining agent edits the file to fix dangling references, align drifting terminology, and supply missing context, without inventing facts. skill-map never writes the body. |
