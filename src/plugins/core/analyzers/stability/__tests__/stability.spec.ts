...
import { stabilityAnalyzer } from '../index.js';
...
async function run(nodes: Node[]): Promise<{
  ...
  const issues = await stabilityAnalyzer.evaluate(c);
...
describe('stability analyzer', () => {
  it('emits an experimental chip + info issue when sidecar annotations.stability is experimental', async () => {
    ...
    strictEqual(issues[0]?.analyzerId, 'stability');
  ...
  it('declares analyzer kind in deterministic mode', () => {
    strictEqual(stabilityAnalyzer.kind, 'analyzer');
    strictEqual(stabilityAnalyzer.mode, 'deterministic');
  });
});
