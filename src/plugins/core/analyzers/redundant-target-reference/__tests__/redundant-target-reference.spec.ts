/**
 * Coverage for the `core/redundant-target-reference` built-in rule
 ...
 *   - Unresolved (broken) links: silent (the `broken-ref` rule is the
...
import { redundantTargetReferenceAnalyzer } from '../index.js';
...
describe('core/redundant-target-reference rule', () => {
  it('emits no issues for an empty link list', async () => {
    const issues = await redundantTargetReferenceAnalyzer.evaluate(ctxWith({}));
  ...
  it('does NOT fire for a single occurrence', async () => {
    ...
    const issues = await redundantTargetReferenceAnalyzer.evaluate(
  ...
  it('fires for cross-extractor multi-form (one edge, two occurrences)', async () => {
    ...
    assert.equal(issue.analyzerId, 'redundant-target-reference');
