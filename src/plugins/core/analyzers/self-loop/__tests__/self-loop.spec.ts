/**
 * Coverage for the `core/self-loop` built-in rule
...
import { selfLoopAnalyzer } from '../index.js';
...
describe('core/self-loop rule', () => {
  it('emits no issues when links is empty', async () => {
    const issues = await selfLoopAnalyzer.evaluate(ctxWith({}));
  ...
  it('emits a warn per path-style self-loop', async () => {
    ...
    const issues = await selfLoopAnalyzer.evaluate(
      ctxWith({ nodes: [cmd], links: [selfLoop] }),
    );
    assert.equal(issues.length, 1);
    const issue = issues[0]!;
    assert.equal(issue.severity, 'warn');
    assert.equal(issue.analyzerId, 'self-loop');
