/**
 * Coverage for the `core/job-file-orphan` built-in rule
...
import { jobFileOrphanAnalyzer } from '../index.js';
...
describe('core/job-file-orphan rule', () => {
  it('emits no issues when ctx.orphanJobFiles is absent (legacy callers)', async () => {
    const issues = await jobFileOrphanAnalyzer.evaluate(emptyCtx());
    assert.deepEqual(issues, []);
  });

  it('emits no issues when ctx.orphanJobFiles is empty', async () => {
    const issues = await jobFileOrphanAnalyzer.evaluate(emptyCtx({ orphanJobFiles: [] }));
    assert.deepEqual(issues, []);
  });
  ...
  it('emits one warn issue per orphan path', async () => {
    ...
    const issues = await jobFileOrphanAnalyzer.evaluate(emptyCtx({ orphanJobFiles: orphans }));
    assert.equal(issues.length, 2);
    for (const [i, issue] of issues.entries()) {
      assert.equal(issue.analyzerId, 'job-file-orphan');
  ...
  it('is pure, the same input yields the same issues across two evaluations', async () => {
    ...
    const a = await jobFileOrphanAnalyzer.evaluate(emptyCtx({ orphanJobFiles: orphans }));
    const b = await jobFileOrphanAnalyzer.evaluate(emptyCtx({ orphanJobFiles: orphans }));
