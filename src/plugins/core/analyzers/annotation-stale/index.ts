 ...
 * two ICON surfaces (`node.sidecar.status` ∈ {`stale-body`,
 ...
 *
 * It emits NO issue (user call 2026-07-20, post-graduation review): the
 * per-node `info` row in the inspector's Findings section duplicated
 * what the clock icons already say, so drift is icon-only now. The chips'
 * tooltips carry the per-face detail (body / frontmatter / both).
 *
...
export const annotationStaleAnalyzer: IBuiltInManifest<IAnalyzer> = {
  ...
  evaluate(ctx: IAnalyzerContext): Issue[] {
    for (const node of ctx.nodes) {
      ...
      // severity: drift is neutral and icon-only (no issue emitted), so
      // it stays out of the card's warn chip and never fails `sm check`.
