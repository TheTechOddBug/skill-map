...
describe('broken-ref, hint when a same-named file lacks frontmatter.name', () => {
  it('@c broken + c.md without name -> finding carries data.hint and fix.summary', async () => {
    const fixture = freshFixture('hint-at-single');
    ...
    const result = await scan(fixture);
    const brokenRefs = result.issues.filter((i) => i.analyzerId === 'broken-ref');
    assert.equal(brokenRefs.length, 1, 'expected exactly one broken-ref');
    const issue = brokenRefs[0]!;
    const hint = issue.data?.['hint'] as Record<string, unknown> | undefined;
    assert.ok(hint, 'finding must carry data.hint');
    assert.equal(hint['kind'], 'missing-frontmatter-name');
    assert.equal(hint['suggestedName'], 'c');
    assert.deepEqual(hint['candidates'], ['.claude/agents/c.md']);
    assert.ok(issue.fix, 'finding must carry fix block');
    assert.equal(issue.fix!.autofixable, false);
    assert.match(issue.fix!.summary!, /name: c/);
    assert.match(issue.fix!.summary!, /\.claude\/agents\/c\.md/);
  });

  it('/c broken + c.md without name -> hint applies to slash sigil too', async () => {
    ...
    const issue = brokenRefs[0]!;
    const hint = issue.data?.['hint'] as Record<string, unknown> | undefined;
    assert.ok(hint, '/c must produce the same hint shape as @c');
    assert.equal(hint['suggestedName'], 'c');
    assert.deepEqual(hint['candidates'], ['.claude/commands/c.md']);
  });

  it('@c broken with no candidate file -> finding has NO hint (regression guard)', async () => {
    ...
    assert.equal(
      issue.data?.['hint'],
      undefined,
      'no candidate file means no hint should be attached',
    );
    assert.equal(issue.fix, undefined, 'no candidate means no fix block either');
  });

  it('two candidate files with the same basename -> candidates lists both', async () => {
    ...
    const brokenRefs = result.issues.filter((i) => i.analyzerId === 'broken-ref');
    assert.equal(brokenRefs.length, 1);
    const hint = brokenRefs[0]!.data?.['hint'] as Record<string, unknown> | undefined;
    assert.ok(hint);
    const candidates = hint['candidates'] as string[];
    assert.equal(candidates.length, 2, 'both files must be listed');
    assert.ok(candidates.includes('.claude/agents/c.md'));
    assert.ok(candidates.includes('.claude/commands/c.md'));
    // Plural summary lists both paths.
    const summary = brokenRefs[0]!.fix!.summary!;
    assert.match(summary, /\.claude\/agents\/c\.md/);
    assert.match(summary, /\.claude\/commands\/c\.md/);
  });

  it('candidate file that DOES advertise name is NOT a hint target', async () => {
    const fixture = freshFixture('hint-with-name');
    // c.md advertises a DIFFERENT name, so @c stays broken AND the file
    // does NOT qualify as a hint candidate (the author already wrote a
    ...
    assert.equal(brokenRefs.length, 1);
    assert.equal(
      brokenRefs[0]!.data?.['hint'],
      undefined,
      'a file that already advertises a name must not surface as a hint',
    );
  });
});
