/**
 * Unit coverage for the dual surface of `broken-ref`:
...
import { brokenRefAnalyzer } from '../index.js';
import { BROKEN_REF_TEXTS } from '../text.js';
...
function run(nodes: Node[], links: Link[]): {
  ...
  const result = brokenRefAnalyzer.evaluate({
    nodes,
    links,
