/**
 * Built-in probabilistic `ai-trigger-action` Action, the fixer paired with
 * the `core/ai-trigger-analyzer` finder (Modelo B, one of the five
 * OPTIMIZATION pairs, 2026-07-22). Resolves that finder's `trigger`
 ...
 * 2026-07-22 in lock-step with its finder); disable with
 * `sm plugins disable core/ai-trigger-action`.
 */

import type { IAction, IBuiltInManifest } from '../../../../kernel/extensions/index.js';
import { CORE_PLUGIN_ID as PLUGIN_ID } from '../../../ids.js';

export const aiTriggerAction: IBuiltInManifest<IAction> = {
  id: 'ai-trigger-action',
  pluginId: PLUGIN_ID,
  kind: 'action',
  description:
    'Fixes the description a review flagged: rewrites it to say what the file really does and when to use it, never touching the content itself.',
