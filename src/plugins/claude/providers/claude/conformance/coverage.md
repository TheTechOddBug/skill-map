...
| `backtick-trigger-extraction` | Backticked triggers produce exactly FOUR resolved links: `@reviewer` (span + fenced repeat deduped, `mentions` to the agent), `@deploy-site` (`mentions` to the skill) and `@playbook` (`mentions` to the markdown doc, both via the priority-ordered matrix), `/deploy-site` (`invokes` to the skill via `core/backtick-slash`); the npm-scope and shell-path baits are pruned by the resolution gate (no links, no broken issues; the mention + invocation pair on one target keeps the standing `reference-redundant` warn). Exercises `spec/architecture.md` §Extractor · code-region triggers end-to-end. | `backtick-trigger` |
