 ...
 * and exits, so re-reading the layers per command costs nothing. The
 ...
 *     the `confirm: true` arm of the sidecar consent gate) call this
...
export class ConfigService {
   ...
   * (PATCH project-preferences) and by the sidecar consent gate after
   * it flips `allowEditSmFiles` to `true`.
   */
  reload(): void {
    this.#cache = null;
  }
