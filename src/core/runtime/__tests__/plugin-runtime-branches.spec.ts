...
describe('plugin-runtime, branch coverage', () => {
  ...
  describe('per-extension toggle filter', () => {
    ...
    it('(c) default: every built-in runs except experimental ones', () => {
      ...
      assert.ok(composed);
      assert.equal(composed.providers.length, 4, 'claude + openai (beta) + agent-skills (stable, locked) + core-markdown load by default; antigravity is experimental so it ships disabled');
  ...
  describe('conformance kill-switches (composer options)', () => {
    ...
    it('(b) killSwitches.extractors empties only the extractors bucket', () => {
      ...
      assert.ok(composed);
      assert.equal(composed.providers.length, 4, 'providers untouched (4: claude + openai (beta) + agent-skills (stable, locked) + core-markdown load; antigravity is experimental so it ships disabled)');
