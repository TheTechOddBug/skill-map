...
describe('plugin-runtime, branch coverage', () => {
  ...
  describe('per-extension toggle filter', () => {
    ...
    it('(c) default: every built-in runs except experimental ones', () => {
      ...
      assert.equal(composed.analyzers.length, 20, 'all 20 analyzers load by default; after core/annotation-stale graduated to stable (2026-07-19) no built-in analyzer is experimental, so every analyzer ships enabled, while the three probabilistic finders (ai-redundancy-analyzer / ai-contradiction-analyzer / ai-incoherence-analyzer) are STABLE queue targets excluded from scan-time phases by the mode gate (the former projector analyzers core/supersede + core/tags were deleted; the remaining inspector buttons self-project from their actions and tag editing moved inline; core/score-resolution was deleted, the kernel now seeds the 1.0 baseline directly; core/job-file-orphan was removed, to return under a probabilistic evaluation model; core/name-mismatch joined for declared-vs-path-handle divergences; core/contribution-orphan, the never-implemented stub, was deleted 2026-07-22; ai-verbosity-analyzer and ai-vagueness-analyzer graduated stable/enabled the same day, their three optimization siblings stay experimental)');
  ...
  describe('conformance kill-switches (composer options)', () => {
    it('(a) killSwitches.providers empties only the providers bucket', () => {
      ...
      assert.equal(composed.analyzers.length, 20, 'analyzers untouched (all 20: core/annotation-stale graduated to stable so no built-in analyzer is experimental, and the three probabilistic finders are STABLE queue targets; the projector analyzers core/supersede + core/tags were deleted; core/score-resolution was deleted, the kernel seeds the 1.0 baseline directly; core/job-file-orphan was removed; core/name-mismatch joined; the contribution-orphan stub was deleted)');
    });
    ...
    it('(b) killSwitches.extractors empties only the extractors bucket', () => {
      ...
      assert.equal(composed.analyzers.length, 20, 'analyzers untouched (all 20: no built-in analyzer is experimental since core/annotation-stale graduated to stable; the three probabilistic finders are stable queue targets)');
    });
