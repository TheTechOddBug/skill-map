...
async function runOnce(args: IRunOnceArgs): Promise<IRunOnceResult> {
  ...
  try {
    ...
    await persistScanResult(adapter.db, ran.result, {
      ...
      contributions: ran.contributions,
      registeredContributionKeys: collectKeys(args.extensions),
      freshlyRunTuples: ran.freshlyRunTuples,
    });
...
describe('sidecar-aware per-(node, extractor) cache key', () => {
  ...
  it('B, end-to-end: flipping annotations.stability propagates through the built-in core/node-stability analyzer', async () => {
    ...
    deepStrictEqual(
      ...
      ['experimental'],
      "first sidecar (experimental) wrote the 'experimental' chip (the Set-stability button now self-projects from the node-set-stability action, not this analyzer)",
    );
    ...
    deepStrictEqual(
      readStabilityContributions(),
      ['deprecated'],
      "post-flip scan replaced the chip experimental -> deprecated (pre-fix bug reused ['experimental'] from cache)",
    );
  });
});
