...
describe('resolveSignalsAgainstNodes', () => {
  ...
  it('normalises the signal name like link resolution does', () => {
    ...
    assert.equal(resolved[0]?.nodePath, '.claude/skills/deploy/SKILL.md');
    assert.equal(resolved[0]?.owner, undefined);
  });
...
describe('resolveActivityEvent (DB-free short circuits)', () => {
  it('unknown provider id yields no signals', async () => {
  ...
  it('provider without an activity capability yields no signals', async () => {
    const resolved = await resolveActivityEvent({
      providers: [makeProvider()],
      dbPath: '/nonexistent/skill-map.db',
      providerId: 'claude',
