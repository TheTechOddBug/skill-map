...
describe('POST /api/activity, legacy agentEndpoint tolerance', () => {
  it('accepts (and ignores) the agentEndpoint field stale generated plugins still send', async () => {
    // Plugins generated before the agent doorbell was removed stamp
    // `agentEndpoint` on every ingest; the schema tolerates it so their
    // activity keeps flowing until `sm activity install` regenerates them.
    await bootAndUse(async (handle) => {
      const res = await postActivity(
        handle,
        {
