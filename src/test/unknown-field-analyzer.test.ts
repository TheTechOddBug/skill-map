 ...
 *
 * The rule is invoked directly with a hand-built `IAnalyzerContext` rather
...
import { unknownFieldAnalyzer } from '../built-in-plugins/analyzers/unknown-field/index.js';
...
function evaluate(
  ...
  const out = unknownFieldAnalyzer.evaluate({
    nodes: [node],
    links: [],
    sidecarRoots,
...
describe('core/unknown-field rule (Step 9.6.6)', () => {
  ...
  it("typo'd key inside annotations: yields exactly 1 warning", () => {
    ...
    strictEqual(issues[0]!.analyzerId, 'unknown-field');
  ...
  it('absent sidecarRoots map → empty issue list (no false positives)', () => {
    const out = unknownFieldAnalyzer.evaluate({ nodes: [fakeNode('a.md')], links: [], emitContribution: () => undefined });
