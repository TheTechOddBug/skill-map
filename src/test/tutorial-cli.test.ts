...
describe('sm tutorial, happy path', () => {
  it('writes sm-tutorial.md in cwd with exit 0 and the success line', () => {
    ...
    assert.match(r.stdout, /sm-tutorial\.md created at/);
    assert.match(r.stdout, /Open Claude Code/);
    assert.match(r.stdout, /@sm-tutorial\.md/);
  });
  ...
  it('content matches the canonical SKILL.md byte-for-byte', () => {
    ...
    assert.deepEqual(
      written,
      source,
      'sm-tutorial.md content must match .claude/skills/sm-tutorial/SKILL.md byte-for-byte',
    );
  });
