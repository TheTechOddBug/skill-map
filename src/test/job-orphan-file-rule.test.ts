 ...
 * (`built-in-plugins/rules/job-orphan-file/index.ts`).
 *
...
import { jobOrphanFileRule } from '../built-in-plugins/rules/job-orphan-file/index.js';
import type { IRuleContext } from '../kernel/extensions/index.js';

function emptyCtx(overrides: Partial<IRuleContext> = {}): IRuleContext {
  return {
    nodes: [],
    links: [],
    emitContribution: () => {
...
describe('core/job-orphan-file rule', () => {
  it('emits no issues when ctx.orphanJobFiles is absent (legacy callers)', async () => {
    const issues = await jobOrphanFileRule.evaluate(emptyCtx());
    assert.deepEqual(issues, []);
  });

  it('emits no issues when ctx.orphanJobFiles is empty', async () => {
    const issues = await jobOrphanFileRule.evaluate(emptyCtx({ orphanJobFiles: [] }));
    assert.deepEqual(issues, []);
  });
  ...
  it('emits one warn issue per orphan path', async () => {
    ...
    const issues = await jobOrphanFileRule.evaluate(emptyCtx({ orphanJobFiles: orphans }));
    assert.equal(issues.length, 2);
    for (const [i, issue] of issues.entries()) {
      assert.equal(issue.ruleId, 'job-orphan-file');
  ...
  it('is pure — the same input yields the same issues across two evaluations', async () => {
    ...
    const a = await jobOrphanFileRule.evaluate(emptyCtx({ orphanJobFiles: orphans }));
    const b = await jobOrphanFileRule.evaluate(emptyCtx({ orphanJobFiles: orphans }));
